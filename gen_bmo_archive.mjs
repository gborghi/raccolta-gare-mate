// Generates KG notes for UK BMO archive from the staging JSON produced by the
// bmo-extract workflow. Mirrors gen_bmo.mjs / build_kg_math.py conventions.
// Idempotent: overwrites notes for the same src, never touches other notes.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/bmo_extract/json"
...
const PDFBASE = "gare_di_altri_paesi/UK/BMO12/archive"

// figures dropped by the figure-pass: _attachments/<src>__Q<nn>*.png|jpg
let attFiles = []
...
// QA scores per src (from the QA workflow)
const QA = "E:/giovanni/bmo_extract/qa"
const qaScore = {}
try {
...
for (const file of files) {
  ...
  const { src, round, year, season, problems } = paper
  if (!src || !Array.isArray(problems) || !problems.length) { bad.push(`${file}: empty/invalid`); continue }
  const title = `British Mathematical Olympiad ${season} — Round ${round}`
  const level = `BMO Round ${round}`
  const pdf = `${PDFBASE}/bmo${round}-${season.slice(0, 4) === String(year) ? Number(season.slice(0, 4)) + 1 : year}.pdf`
  const index = []
  for (const q of problems) {
    ...
    const tags = ["kg/quesito", "paese/regno-unito", "comp/bmo", `anno/${year}`, `livello/${tagSlug(level)}`]
    ...
    const fm = ["---", "tipo: quesito", `quesito_id: quesito_${qfile}`, `parent: ${src}`,
      `competition: "${title}"`, "family: bmo", `year: ${year}`, `level: "${level}"`,
      "country: Regno Unito", 'modalita: "individuale"', `quesito: ${pn}`, `summary: "${ysafe(q.summary || "")}"`]
    ...
    if (q.answer) body.push(`**Risposta:** ${q.answer}`)
    body.push(`**Fonte:** [apri PDF](<../../${pdf}>)`)
    if (score != null && score <= 2) body.push("", "> [!warning] Estratto da scansione a bassa risoluzione — enunciato da verificare con la fonte.")
    await fs.writeFile(path.join(QDIR, qfile + ".md"), body.join("\n").trimEnd() + "\n")
    index.push([pn, qfile, q.summary || ""])
    nQ++
  }
  const gi = ["---", "tipo: gara", `src_id: ${src}`, `competition: "${title}"`, "family: bmo",
    `year: ${year}`, `level: "${level}"`, "country: Regno Unito", 'modalita: "individuale"',
    `n_quesiti: ${index.length}`, `tags: [kg/gara, paese/regno-unito, comp/bmo, anno/${year}, gara/individuale]`,
    "---", "", `# ${title}`, "", `Sorgente: [apri PDF](<../../${pdf}>)\n`]

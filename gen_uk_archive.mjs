// Generates KG notes for the UK British Mathematical Olympiad (BMO) archive
// from uk_extract/json (already English). country: Regno Unito. family: bmo.
// Rounds 1 and 2; one note per problem, one gara note per round-year.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/uk_extract/json"
const MANIFEST = "E:/giovanni/uk_extract/manifest.json"
const QA = "E:/giovanni/uk_extract/qa"
...
const PDFBASE = "gare_di_altri_paesi/UK"
...
for (const file of files) {
  ...
  const { src, year, round, problems } = paper
  if (!src || !Array.isArray(problems) || !problems.length) { bad.push(`${file}: empty/invalid`); continue }
  const rd = round || (src.includes("bmo2") ? 2 : 1)
  const title = `British Mathematical Olympiad ${year} — Round ${rd}`
  const level = `BMO Round ${rd}`
  ...
  for (const q of problems) {
    ...
    const tags = ["kg/quesito", "paese/regno-unito", "comp/bmo", `anno/${year}`, `livello/${tagSlug(level)}`, `round/${rd}`]
    ...
    const fm = ["---", `title: "${ysafe(title)} — Quesito ${pn}"`, "tipo: quesito", `quesito_id: quesito_${qfile}`, `parent: ${src}`,
      `competition: "${title}"`, "family: bmo", `year: ${year}`, `level: "${level}"`, `round: ${rd}`,
  ...
  const tags = ["kg/gara", "paese/regno-unito", "comp/bmo", `anno/${year}`, `round/${rd}`, "gara/individuale"]
  const gi = ["---", `title: "${ysafe(title)}"`, "tipo: gara", `src_id: ${src}`, `competition: "${title}"`, "family: bmo",
    `year: ${year}`, `level: "${level}"`, `round: ${rd}`, "country: Regno Unito", 'modalita: "individuale"',

// Generates KG notes for the IMO archive from imho_extract/json (English).
// Replaces any existing src_imho_* notes. country: Italia, family: imo, level IMO.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/imho_extract/json"
const MANIFEST = "E:/giovanni/imho_extract/manifest.json"
const QA = "E:/giovanni/imho_extract/qa"
...
const editionOf = (year) => year - 1958 - (year >= 1981 ? 1 : 0)  // 1959=1 ... 1979=21, 1981=22 ... (no 1980)
...
// LEAVE existing src_imho_* notes untouched; only ADD missing years.
const existingNotes = new Set(await fs.readdir(QDIR))
const yearExists = (src) => existingNotes.has(`${src}.md`)
...
for (const file of files) {
  ...
  catch (e) { bad.push(`${file}: ${e.message}`); continue }
  const { src, year, problems } = paper
  if (!src || !Array.isArray(problems) || !problems.length) { bad.push(`${file}: empty`); continue }
  if (yearExists(src)) { continue }  // already in KG (e.g. 1959-1964) — do not touch
  const ed = editionOf(year)
  const title = `IMO ${year} (${roman(ed)} International Mathematical Olympiad)`
  const pdf = fileFor[src] || ""
  const score = qaScore[src]
  const index = []
  for (const q of problems) {
    const n = q.n
    const qfile = `${src}__Q${pad(n)}`
    const topics = q.topics || [], methods = q.methods || [], skills = q.skills || []
    const tags = ["kg/quesito", "paese/italia", "comp/imo", `anno/${year}`, "livello/IMO"]
    ...
    const fm = ["---", `title: "${ysafe(title)} — Quesito ${n}"`, "tipo: quesito", `quesito_id: quesito_${qfile}`,
      `parent: ${src}`, `competition: "${title}"`, "family: imo", `year: ${year}`, 'level: "IMO"',
      "country: Italia", 'modalita: "individuale"', `quesito: ${n}`, `summary: "${ysafe(q.summary || "")}"`]
    ...
    const pg = figPage[`${src}__${n}`] || 1
    if (pdf) body.push(`**Fonte:** [apri PDF p.${pg}](<../../${pdf}#page=${pg}>)`)
  ...
  const tags = ["kg/gara", "paese/italia", "comp/imo", `anno/${year}`, "gara/individuale"]
  const gi = ["---", `title: "${ysafe(title)}"`, "tipo: gara", `src_id: ${src}`, `competition: "${title}"`,
    "family: imo", `year: ${year}`, 'level: "IMO"', "country: Italia", 'modalita: "individuale"',
    `n_quesiti: ${index.length}`, `tags: [${tags.join(", ")}]`, "---", ""]
  if (pdf) gi.push(`Sorgente: [apri PDF](<../../${pdf}>)\n`)
  for (const [n, qfile, summ] of index.sort((a, b) => a[0] - b[0])) gi.push(`- [[${qfile}|Quesito ${n}]] — ${summ}`)
  await fs.writeFile(path.join(QDIR, src + ".md"), gi.join("\n").trimEnd() + "\n")
  nG++
}
console.log(`generated ${nQ} quesiti + ${nG} gare from ${files.length} json`)

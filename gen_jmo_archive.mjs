// Generates KG notes for the Japan archive from jmo_extract/json staging.
// Mirrors gen_bmo_archive.mjs. country: Giappone. Statements are English
// (translated from Japanese). Idempotent per src.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/jmo_extract/json"
const MANIFEST = "E:/giovanni/jmo_extract/manifest.json"
const QA = "E:/giovanni/jmo_extract/qa"
...
const PDFBASE = "gare_di_altri_paesi/Giappone"

let attFiles = []
try { attFiles = await fs.readdir(ATT) } catch {}
const figuresFor = (qfile) =>
  attFiles.filter((f) => f.startsWith(qfile) && /\.(png|jpe?g)$/i.test(f)).sort()
...
const COMP_LABEL = { jmo: "Japan Mathematical Olympiad", jjmo: "Japan Junior Mathematical Olympiad", jegmo: "Japan EGMO Selection / Girls MO", camp: "Japan IMO Selection Camp" }
const ROUND_LABEL = { yosen: "Yosen (Preliminary)", honsen: "Honsen (Final)", camp: "Selection Camp", single: "" }
const COMP_TAG = { jmo: "jmo", jjmo: "jjmo", jegmo: "jegmo", camp: "jcamp" }
...
for (const file of files) {
  ...
  const { src, comp, round, year, problems } = paper
  if (!src || !Array.isArray(problems) || !problems.length) { bad.push(`${file}: empty/invalid`); continue }
  const rlab = ROUND_LABEL[round] || ""
  const title = `${COMP_LABEL[comp] || comp} ${year}${rlab ? " — " + rlab : ""}`
  const level = `${(comp || "").toUpperCase()}${rlab ? " " + (round === "yosen" ? "Yosen" : round === "honsen" ? "Honsen" : rlab) : ""}`.trim()
  const ctag = COMP_TAG[comp] || comp
  const pdf = `${PDFBASE}/${fileFor[src] || ""}`
  const index = []
  for (const q of problems) {
    ...
    const tags = ["kg/quesito", "paese/giappone", `comp/${ctag}`, `anno/${year}`, `livello/${tagSlug(level)}`]
    ...
    const fm = ["---", "tipo: quesito", `quesito_id: quesito_${qfile}`, `parent: ${src}`,
      `competition: "${title}"`, `family: ${ctag}`, `year: ${year}`, `level: "${level}"`,
      "country: Giappone", 'modalita: "individuale"', `quesito: ${pn}`, `summary: "${ysafe(q.summary || "")}"`]
    ...
    if (score != null && score <= 2) body.push("", "> [!warning] Estratto da scansione giapponese a bassa risoluzione — enunciato tradotto da verificare con la fonte.")
    await fs.writeFile(path.join(QDIR, qfile + ".md"), body.join("\n").trimEnd() + "\n")
    index.push([pn, qfile, q.summary || ""])
    nQ++
  }
  const gi = ["---", "tipo: gara", `src_id: ${src}`, `competition: "${title}"`, `family: ${ctag}`,
    `year: ${year}`, `level: "${level}"`, "country: Giappone", 'modalita: "individuale"',
    `n_quesiti: ${index.length}`, `tags: [kg/gara, paese/giappone, comp/${ctag}, anno/${year}, gara/individuale]`,
    "---", "", `# ${title}`, ""]

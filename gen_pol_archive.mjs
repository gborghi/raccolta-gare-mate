// Generates KG notes for the Polish Mathematical Olympiad archive
// from pol_extract/json (already English, imomath). country: Polonia. family: pol.
// Rounds 1/2/3; one note per problem, one gara note per round-year.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/pol_extract/json"
const MANIFEST = "E:/giovanni/pol_extract/manifest.json"
const QA = "E:/giovanni/pol_extract/qa"
...
const PDFBASE = "gare_di_altri_paesi/Polonia"
...
const tagSlug = (s) => s.toLowerCase().replace(/\s+/g, "-")
...
for (const file of files) {
  ...
  const rd = round || 3
  const title = `Olimpiade Matematica Polacca ${year} — Round ${rd}`
  const level = `Olimpiade Polacca Round ${rd}`
  ...
  for (const q of problems) {
    ...
    const tags = ["kg/quesito", "paese/polonia", "comp/pol", `anno/${year}`, `livello/${tagSlug(level)}`, `round/${rd}`]
    ...
    const fm = ["---", `title: "${ysafe(title)} — Quesito ${pn}"`, "tipo: quesito", `quesito_id: quesito_${qfile}`, `parent: ${src}`,
      `competition: "${title}"`, "family: pol", `year: ${year}`, `level: "${level}"`, `round: ${rd}`,
      "country: Polonia", 'modalita: "individuale"', `quesito: ${pn}`, `summary: "${ysafe(q.summary || "")}"`]
  ...
  const tags = ["kg/gara", "paese/polonia", "comp/pol", `anno/${year}`, `round/${rd}`, "gara/individuale"]
  const gi = ["---", `title: "${ysafe(title)}"`, "tipo: gara", `src_id: ${src}`, `competition: "${title}"`, "family: pol",
    `year: ${year}`, `level: "${level}"`, `round: ${rd}`, "country: Polonia", 'modalita: "individuale"',

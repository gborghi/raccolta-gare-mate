// Generates KG notes for the OBM archive (Olimpíada Brasileira de Matemática)
// from obm_extract/json. English statements (translated from Portuguese).
// country: Brasile. family: obm. level encodes Nível; fase tagged.
import { promises as fs } from "node:fs"
import path from "node:path"

const STAGING = "E:/giovanni/obm_extract/json"
const MANIFEST = "E:/giovanni/obm_extract/manifest.json"
const QA = "E:/giovanni/obm_extract/qa"
...
const PDFBASE = "gare_di_altri_paesi/Brasil/OBM"
...
const NIVEL_LABEL = { "1": "Nível 1", "2": "Nível 2", "3": "Nível 3", "u": "Nível Universitário" }
...
for (const file of files) {
  ...
  const { src, year, nivel, fase, problems } = paper
  if (!src || !Array.isArray(problems) || !problems.length) { bad.push(`${file}: empty/invalid`); continue }
  const nlab = NIVEL_LABEL[nivel] || `Nível ${nivel}`
  const flab = fase ? ` — Fase ${fase}` : ""
  const title = `Olimpíada Brasileira de Matemática ${year} — ${nlab}${flab}`
  const level = `OBM ${nlab}`
  ...
  for (const q of problems) {
    ...
    const tags = ["kg/quesito", "paese/brasile", "comp/obm", `anno/${year}`, `livello/${tagSlug(level)}`, `nivel/${nivel}`]
    if (fase) tags.push(`fase/${fase}`)
    ...
    const fm = ["---", "tipo: quesito", `quesito_id: quesito_${qfile}`, `parent: ${src}`,
      `competition: "${title}"`, "family: obm", `year: ${year}`, `level: "${level}"`,
      "country: Brasile", 'modalita: "individuale"', `nivel: "${nivel}"`]
    if (fase) fm.push(`fase: "${fase}"`)
  ...
  const tags = ["kg/gara", "paese/brasile", "comp/obm", `anno/${year}`, `nivel/${nivel}`, "gara/individuale"]
  if (fase) tags.push(`fase/${fase}`)
  const gi = ["---", "tipo: gara", `src_id: ${src}`, `competition: "${title}"`, "family: obm",
    `year: ${year}`, `level: "${level}"`, "country: Brasile", 'modalita: "individuale"',
    `nivel: "${nivel}"`, ...(fase ? [`fase: "${fase}"`] : []),

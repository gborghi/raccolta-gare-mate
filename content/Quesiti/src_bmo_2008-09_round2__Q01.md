---
tipo: quesito
quesito_id: quesito_src_bmo_2008-09_round2__Q01
parent: src_bmo_2008-09_round2
competition: British Mathematical Olympiad 2008-09 — Round 2
family: bmo
year: '2009'
level: BMO Round 2
country: Regno Unito
modalita: individuale
quesito: '1'
summary: Find non-negative integer solutions to sum of square roots equals 2009
qa_score: '5'
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/bmo
  - anno/2009
  - livello/BMO-Round-2
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# British Mathematical Olympiad 2008-09 — Round 2 — Quesito 1

*Find non-negative integer solutions to sum of square roots equals 2009*

> Find all solutions in non-negative integers $a, b, c$ to $\sqrt{a} + \sqrt{b} + \sqrt{c} = 2009$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Fonte:** apri PDF

---
title: Olimpiade Matematica Polacca 1998 — Round 1 — Quesito 1
tipo: quesito
quesito_id: quesito_src_pol_1998_r1__Q01
parent: src_pol_1998_r1
competition: Olimpiade Matematica Polacca 1998 — Round 1
family: pol
year: '1998'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '1'
summary: Solve a system of two equations involving absolute values
qa_score: '4'
topics:
  - topic_algebra
methods:
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_casework_accurato
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1998
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/algebra
  - gara/individuale
---

*Solve a system of two equations involving absolute values*

> Solve the system of equations:
> $$|x - y| - \frac{|y|}{x} = -1,$$
> $$|2x - y| + |x + y - 1| + |x - y| - y + x - 1 = 0.$$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_casework_accurato|Casework accurato]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

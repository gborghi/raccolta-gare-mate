---
tipo: quesito
quesito_id: quesito_src_obm_2001_n2_f1__Q25
parent: src_obm_2001_n2_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1
family: obm
year: '2001'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '25'
summary: Length FA in a circumscribable hexagon given five consecutive side lengths
qa_score: '2'
answer: B
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/geometria_piana
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1 — Quesito 25

*Length FA in a circumscribable hexagon given five consecutive side lengths*

> The hexagon $ABCDEF$ is circumscribable. If $AB = 1$, $BC = 2$, $CD = 3$, $DE = 4$ and $EF = 5$, how much does $FA$ measure?
> 
> (A) $1$ \quad (B) $3$ \quad (C) $15/8$ \quad (D) $6$ \quad (E) $9$

![[src_obm_2001_n2_f1__Q25.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Risposta:** B
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

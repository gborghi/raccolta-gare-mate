---
title: Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1 — Quesito 13
tipo: quesito
quesito_id: quesito_src_obm_1998_n2_f1__Q13
parent: src_obm_1998_n2_f1
competition: Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1
family: obm
year: '1998'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '13'
summary: Find ticket price per student given total and absences
qa_score: '3'
answer: E
topics:
  - topic_algebra
skills:
  - skill_modellizzazione
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1998
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/algebra
  - gara/individuale
---

*Find ticket price per student given total and absences*

> To attend the film \textit{Central do Brasil}, each of $x$ students from a class should pay $y$ reais for the bus fare. Since 3 students were absent, each of the students present had to pay 2 reais more to cover the total cost. What was the price per person?
> 
> A) $(x+3)(y-2)$
> B) $(x-3)y + 2$
> C) $x(y-2) - 3$
> D) $xy - 6$
> E) $(x-3)(y+2)$

**Topic:** [[topic_algebra|Algebra]]
**Abilita:** [[skill_modellizzazione|Modellizzazione]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** E
**Fonte:** [apri PDF](https://drive.google.com/file/d/13b_FsjYtLvDLsjdCbbU3453Jhq-KhThw/view)

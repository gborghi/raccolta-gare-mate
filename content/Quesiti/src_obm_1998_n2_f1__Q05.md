---
tipo: quesito
quesito_id: quesito_src_obm_1998_n2_f1__Q05
parent: src_obm_1998_n2_f1
competition: Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1
family: obm
year: '1998'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '5'
summary: Area of triangle with vertices at midpoints of square sides
qa_score: '3'
answer: B
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1998
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/geometria_piana
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1 — Quesito 5

*Area of triangle with vertices at midpoints of square sides*

> The quadrilateral $ABCD$ is a square of area $4\,\mathrm{m}^2$. Points $M$ and $N$ are the midpoints of sides $DC$ and $BC$, respectively. What is the area, in $\mathrm{m}^2$, of the shaded triangle (the triangle with vertices at $A$, $M$, and $N$)?
> 
> A) 2
> B) 1.5
> C) 2.5
> D) 3
> E) 3.5

![[src_obm_1998_n2_f1__Q05.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Risposta:** B
**Fonte:** apri PDF

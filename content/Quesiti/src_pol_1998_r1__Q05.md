---
title: Olimpiade Matematica Polacca 1998 — Round 1 — Quesito 5
tipo: quesito
quesito_id: quesito_src_pol_1998_r1__Q05
parent: src_pol_1998_r1
competition: Olimpiade Matematica Polacca 1998 — Round 1
family: pol
year: '1998'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '5'
summary: Solve a trigonometric equation depending on a positive integer n
qa_score: '4'
topics:
  - topic_trigonometria
methods:
  - method_trigonometria
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1998
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/trigonometria
  - gara/individuale
---

*Solve a trigonometric equation depending on a positive integer n*

> Solve the equation $|\tan^2 x - \cot^2 x| = 2n|\cot 2x|$, where $n$ is a given positive integer.

**Topic:** [[topic_trigonometria|Trigonometria]]
**Metodo:** [[method_trigonometria|Trigonometria]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Fonte:** apri PDF

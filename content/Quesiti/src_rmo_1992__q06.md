---
title: Regional Mathematical Olympiad 1992 — Quesito 6
tipo: quesito
quesito_id: quesito_src_rmo_1992__Q06
parent: src_rmo_1992
competition: Regional Mathematical Olympiad 1992
family: rmo
year: '1992'
level: RMO
country: India
modalita: individuale
quesito: '6'
summary: >-
  Quadrilateral ABCD with midpoints P,Q of CD,AB; lines meet; prove area
  ADX+area BCY=area PXQY.
qa_score: '4'
topics:
  - topic_geometria_piana
methods:
  - method_coordinate
  - method_simmetria
skills:
  - skill_ragionamento_geometrico
  - skill_modellizzazione
tags:
  - kg/quesito
  - paese/india
  - comp/rmo
  - anno/1992
  - livello/RMO
  - topic/geometria_piana
  - gara/individuale
---

*Quadrilateral ABCD with midpoints P,Q of CD,AB; lines meet; prove area ADX+area BCY=area PXQY.*

> $ABCD$ is a quadrilateral and $P$, $Q$ are mid-points of $CD$, $AB$ respectively. Let $AP$, $DQ$ meet at $X$, and $BP$, $CQ$ meet at $Y$. Prove that
> $$\text{area of } ADX + \text{area of } BCY = \text{area of quadrilateral } PXQY.$$

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_coordinate|Coordinate]], [[method_simmetria|Simmetria]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_modellizzazione|Modellizzazione]]
**Area:** [[Geometria]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1E3JJBw_abvMOZ21Bh0CQ7FOsWYT93fGu/view)

---
title: IMO 2019 (LX International Mathematical Olympiad) — Quesito 2
tipo: quesito
quesito_id: quesito_src_imho_2019__Q02
parent: src_imho_2019
competition: IMO 2019 (LX International Mathematical Olympiad)
family: imo
year: '2019'
level: IMO
country: Italia
modalita: individuale
quesito: '2'
summary: >-
  Concyclicity of two points defined via angle and parallel conditions in
  triangle
qa_score: '5'
topics:
  - topic_geometria_piana
methods:
  - method_trigonometria
  - method_simmetria
skills:
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/2019
  - livello/IMO
  - topic/geometria_piana
  - gara/individuale
---

*Concyclicity of two points defined via angle and parallel conditions in triangle*

> In triangle $ABC$, point $A_1$ lies on side $BC$ and point $B_1$ lies on side $AC$. Let $P$ and $Q$ be points on segments $AA_1$ and $BB_1$, respectively, such that $PQ$ is parallel to $AB$. Let $P_1$ be a point on line $PB_1$, such that $B_1$ lies strictly between $P$ and $P_1$, and $\angle PP_1C = \angle BAC$. Let $Q_1$ be a point on line $QA_1$, such that $A_1$ lies strictly between $Q$ and $Q_1$, and $\angle CQ_1Q = \angle CBA$.
> 
> Prove that points $P$, $Q$, $P_1$, and $Q_1$ are concyclic.

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_trigonometria|Trigonometria]], [[method_simmetria|Simmetria]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Fonte:** apri PDF p.1

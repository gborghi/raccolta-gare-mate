---
title: China Southeastern Mathematical Olympiad 2011 — Quesito 7
tipo: quesito
quesito_id: quesito_src_cn_csmo_2011__Q07
parent: src_cn_csmo_2011
competition: China Southeastern Mathematical Olympiad 2011
family: csmo
year: '2011'
level: China Southeastern Mathematical Olympiad
country: Cina
modalita: individuale
quesito: '7'
summary: >-
  Sequence defined by recurrence; prove a_n + 2a_{n+2} is always a perfect
  square
qa_score: '3'
topics:
  - topic_algebra
  - topic_aritmetica
methods:
  - method_induzione
  - method_ricorsione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/cina
  - comp/csmo
  - anno/2011
  - livello/China-Southeastern-Mathematical-Olympiad
  - topic/algebra
  - topic/aritmetica
  - gara/individuale
---

*Sequence defined by recurrence; prove a_n + 2a_{n+2} is always a perfect square*

> Suppose that the sequence $\{a_n\}$ is defined by $a_1 = a_2 = 1$, $a_n = 7a_{n-1} - a_{n-2} + n$ for $n \geq 3$. Prove that $a_n + 2a_{n+2}$ is a perfect square for any positive integer $n$. (posed by Tao Pingsheng)

**Topic:** [[topic_algebra|Algebra]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_induzione|Induzione]], [[method_ricorsione|Ricorsione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]], [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri libro PDF

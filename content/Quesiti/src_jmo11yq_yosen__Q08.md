---
tipo: quesito
quesito_id: quesito_src_jmo11yq_yosen__Q08
parent: src_jmo11yq_yosen
competition: Japan Mathematical Olympiad 2001 — Yosen (Preliminary)
family: jmo
year: '2001'
level: JMO Yosen
country: Giappone
modalita: individuale
quesito: '8'
summary: Common real roots of two polynomial equations
qa_score: '5'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/giappone
  - comp/jmo
  - anno/2001
  - livello/JMO-Yosen
  - topic/algebra
  - gara/individuale
---

# Japan Mathematical Olympiad 2001 — Yosen (Preliminary) — Quesito 8

*Common real roots of two polynomial equations*

> Find all real numbers $x$ that satisfy both of the following equations simultaneously:
> $$x^5 + 2x^4 - x^3 - 5x^2 - 10x + 5 = 0$$
> $$x^6 + 4x^5 + 3x^4 - 6x^3 - 20x^2 - 15x + 5 = 0$$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

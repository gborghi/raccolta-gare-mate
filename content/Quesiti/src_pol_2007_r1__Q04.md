---
title: Olimpiade Matematica Polacca 2007 — Round 1 — Quesito 4
tipo: quesito
quesito_id: quesito_src_pol_2007_r1__Q04
parent: src_pol_2007_r1
competition: Olimpiade Matematica Polacca 2007 — Round 1
family: pol
year: '2007'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '4'
summary: >-
  Determine the number of sequences of length n with terms in {0,...,9} such
  that among any three consecutive terms at least two are equal.
qa_score: '4'
topics:
  - topic_combinatoria
methods:
  - method_ricorsione
  - method_casework
skills:
  - skill_conteggio_sistematico
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/2007
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/combinatoria
  - gara/individuale
---

*Determine the number of sequences of length n with terms in {0,...,9} such that among any three consecutive terms at least two are equal.*

> For every integer $n\ge 3$ determine the number of sequences $(c_1,c_2,\ldots,c_n)$ with terms in $S=\{0,1,2,\ldots,9\}$ which satisfy the following condition: For every three consecutive terms, at least two of them are equal.

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_ricorsione|Ricorsione]], [[method_casework|Casework]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri PDF

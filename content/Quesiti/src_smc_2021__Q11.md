---
title: Senior Mathematical Challenge 2021 — Quesito 11
tipo: quesito
quesito_id: quesito_src_smc_2021__Q11
parent: src_smc_2021
competition: Senior Mathematical Challenge 2021
family: smc
year: '2021'
level: Senior Mathematical Challenge
country: Regno Unito
modalita: scelta multipla
quesito: '11'
summary: >-
  Alinta claims p odd prime implies p² − 2 is also an odd prime; find a
  counterexample value of p.
qa_score: '4'
answer: E
topics:
  - topic_aritmetica
methods:
  - method_casework
skills:
  - skill_lettura_attenta
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/smc
  - anno/2021
  - livello/senior-mathematical-challenge
  - modalita/scelta-multipla
  - topic/aritmetica
  - gara/individuale
---

*Alinta claims p odd prime implies p² − 2 is also an odd prime; find a counterexample value of p.*

> Alinta claims that if $p$ is an odd prime then $p^2 - 2$ is also an odd prime. Which of the following values of $p$ is a counterexample to this claim?
>
> - **(A)** 3
> - **(B)** 5
> - **(C)** 7
> - **(D)** 9
> - **(E)** 11

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** (E)
**Fonte:** apri PDF

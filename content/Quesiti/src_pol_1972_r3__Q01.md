---
title: Olimpiade Matematica Polacca 1972 — Round 3 — Quesito 1
tipo: quesito
quesito_id: quesito_src_pol_1972_r3__Q01
parent: src_pol_1972_r3
competition: Olimpiade Matematica Polacca 1972 — Round 3
family: pol
year: '1972'
level: Olimpiade Polacca Round 3
round: '3'
country: Polonia
modalita: individuale
quesito: '1'
summary: >-
  Three degree-one polynomials whose n-th powers satisfy u1^n+u2^n=u3^n are all
  proportional to Ax+B.
qa_score: '5'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_astrazione
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1972
  - livello/olimpiade-polacca-round-3
  - round/3
  - topic/algebra
  - gara/individuale
---

*Three degree-one polynomials whose n-th powers satisfy u1^n+u2^n=u3^n are all proportional to Ax+B.*

> Polynomials $u_i(x) = a_i x + b_i$ ($a_i, b_i \in \mathbb{R}$, $i = 1, 2, 3$) satisfy
> $$u_1(x)^n + u_2(x)^n = u_3(x)^n \quad \text{for some integer } n \ge 2.$$
> Prove that there exist real numbers $A$, $B$, $c_1$, $c_2$, $c_3$ such that $u_i(x) = c_i(Ax + B)$ for $i = 1, 2, 3$.

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_astrazione|Astrazione]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1u10PZn1Xtd_4pvaqg5qH_EntPwiPBjmg/view)

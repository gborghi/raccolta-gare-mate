---
tipo: quesito
quesito_id: quesito_src_obm_2004_n1_f1__Q22
parent: src_obm_2004_n1_f1
competition: Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1
family: obm
year: '2004'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '22'
summary: Minimum path visiting all nodes of an interconnected map
qa_score: '3'
answer: E
topics:
  - topic_combinatoria
methods:
  - method_grafi
  - method_estremalita
skills:
  - skill_ragionamento_geometrico
  - skill_modellizzazione
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/combinatoria
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1 — Quesito 22

*Minimum path visiting all nodes of an interconnected map*

> A map viewed from above shows a residential neighborhood where all houses, numbered $1$ through $23$, are interconnected by $23$ lanes. The vendor Zé Ruda, who lives in house $8$, plans to pass through all other houses and return home, traveling the minimum number of lanes. How many lanes must he travel?
> 
> (A) $15$ \quad (B) $10$ \quad (C) $12$ \quad (D) $12$ \quad (E) $11$

![[src_obm_2004_n1_f1__Q22.png]]

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_grafi|Grafi]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_modellizzazione|Modellizzazione]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** E
**Fonte:** apri PDF

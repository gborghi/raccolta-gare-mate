---
tipo: quesito
quesito_id: quesito_src_bmo_1994-95_round2__Q03
parent: src_bmo_1994-95_round2
competition: British Mathematical Olympiad 1994-95 — Round 2
family: bmo
year: '1995'
level: BMO Round 2
country: Regno Unito
modalita: individuale
quesito: '3'
summary: 'Prove inequality for real numbers in (0,4)'
qa_score: '4'
topics:
  - topic_disuguaglianze
  - topic_algebra
methods:
  - method_disuguaglianze
skills:
  - skill_manipolazione_algebrica
  - skill_stima
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/bmo
  - anno/1995
  - livello/BMO-Round-2
  - topic/disuguaglianze
  - topic/algebra
  - gara/individuale
---

# British Mathematical Olympiad 1994-95 — Round 2 — Quesito 3

*Prove inequality for real numbers in (0,4)*

> Let $a$, $b$, $c$ be real numbers with $0 < a < b < c < 4$. Prove that $0 < a + b + c - abc < 4$.

**Topic:** [[topic_disuguaglianze|Disuguaglianze]], [[topic_algebra|Algebra]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

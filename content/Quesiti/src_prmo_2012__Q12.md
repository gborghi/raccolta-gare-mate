---
title: Pre-Regional Mathematical Olympiad (PRMO) 2012 — Mumbai — Quesito 12
tipo: quesito
quesito_id: quesito_src_prmo_2012__Q12
parent: src_prmo_2012
competition: Pre-Regional Mathematical Olympiad (PRMO) 2012
family: prmo
year: '2012'
level: PRMO
country: India
modalita: individuale
quesito: '12'
summary: Find m+n given nested radical equals sqrt(m) - sqrt(n)
regioni:
  - Mumbai
qa_score: '3'
answer: '2011'
topics:
  - topic_algebra
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/india
  - comp/prmo
  - anno/2012
  - livello/PRMO
  - topic/algebra
  - regione/Mumbai
  - gara/individuale
---

*Find m+n given nested radical equals sqrt(m) - sqrt(n)*

> If $\dfrac{1}{\sqrt{2011 + \sqrt{2011^2 - 1}}} = \sqrt{m} - \sqrt{n}$, where $m$ and $n$ are positive integers, what is the value of $m + n$?

**Topic:** [[topic_algebra|Algebra]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** 2011
**Fonte:** apri PDF

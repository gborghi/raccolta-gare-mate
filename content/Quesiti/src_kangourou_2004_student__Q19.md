---
tipo: quesito
quesito_id: quesito_src_kangourou_2004_student__Q19
parent: src_kangourou_2004_student
competition: Kangourou 2004 - Student (marzo)
family: kangourou
year: '2004'
level: kangourou
country: Italia
quesito: '19'
summary: Segno prodotto in successione geometrica a3<a2<a4
answer: B
topics:
  - topic_funzionali
  - topic_algebra
methods:
  - method_casework
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/italia
  - comp/kangourou
  - anno/2004
  - livello/kangourou
  - topic/funzionali
  - topic/algebra
---

# Kangourou 2004 - Student (marzo) — Quesito 19

*Segno prodotto in successione geometrica a3<a2<a4*

> In una successione geometrica  a 1, a 2, a 3, ... si hanno le seguenti disuguaglianze: a 3 <  a 2 <  a 4 .
> Allora sicuramente
> A) a 3 Aa 4 > 0                B)a 2 Aa 3 < 0   
> C) a 2 Aa 4 < 0
> D)a 2 < 0   
> E) a 2 Aa 3 > 0

**Topic:** [[topic_funzionali|Equazioni funzionali / successioni]], [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Analisi per casi]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** B
**Fonte:** apri PDF p.3

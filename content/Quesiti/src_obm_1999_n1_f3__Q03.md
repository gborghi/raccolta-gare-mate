---
title: Olimpíada Brasileira de Matemática 1999 — Nível 1 — Fase 3 — Quesito 3
tipo: quesito
quesito_id: quesito_src_obm_1999_n1_f3__Q03
parent: src_obm_1999_n1_f3
competition: Olimpíada Brasileira de Matemática 1999 — Nível 1 — Fase 3
family: obm
year: '1999'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '3'
quesito: '3'
summary: Citizen exiled between cities; can he return to city A?
qa_score: '5'
topics:
  - topic_logica
  - topic_combinatoria
methods:
  - method_invarianti
  - method_estremalita
skills:
  - skill_ragionamento_geometrico
  - skill_astrazione
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1999
  - livello/OBM-Nível-1
  - nivel/1
  - fase/3
  - topic/logica
  - topic/combinatoria
  - gara/individuale
---

*Citizen exiled between cities; can he return to city A?*

> A kingdom is made up of ten cities. A citizen who misbehaves is exiled from city $A$ to city $B$, where $B$ is the city in the kingdom farthest from $A$. After some time, he is expelled from city $B$ to city $C$, the city in the kingdom farthest from $B$. It is known that city $C$ is not the same as city $A$.
> 
> **Note:** Distances between all cities are different.
> 
> If he continues to be exiled in this way, is it possible for him to return to city $A$?

**Topic:** [[topic_logica|Logica]], [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_invarianti|Invarianti]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_astrazione|Astrazione]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri PDF

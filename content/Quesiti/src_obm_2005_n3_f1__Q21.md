---
tipo: quesito
quesito_id: quesito_src_obm_2005_n3_f1__Q21
parent: src_obm_2005_n3_f1
competition: Olimpíada Brasileira de Matemática 2005 — Nível 3 — Fase 1
family: obm
year: '2005'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '21'
summary: 'Order three large/exotic powers A, B, C by size'
qa_score: '2'
topics:
  - topic_algebra
  - topic_disuguaglianze
methods:
  - method_disuguaglianze
skills:
  - skill_stima
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2005
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/algebra
  - topic/disuguaglianze
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2005 — Nível 3 — Fase 1 — Quesito 21

*Order three large/exotic powers A, B, C by size*

> Let $A = 10^{(16/2005)^2}$, $B = 2005^2$ and $C = 2^{\sqrt{2005}}$. Then:
> (A) $A < B < C$ (B) $A < C < B$ (C) $B < A < C$ (D) $B < C < A$ (E) $C < A < B$

**Topic:** [[topic_algebra|Algebra]], [[topic_disuguaglianze|Disuguaglianze]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]]
**Abilita:** [[skill_stima|Stima]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

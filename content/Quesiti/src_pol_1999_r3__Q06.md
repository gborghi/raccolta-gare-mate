---
title: Olimpiade Matematica Polacca 1999 — Round 3 — Quesito 6
tipo: quesito
quesito_id: quesito_src_pol_1999_r3__Q06
parent: src_pol_1999_r3
competition: Olimpiade Matematica Polacca 1999 — Round 3
family: pol
year: '1999'
level: Olimpiade Polacca Round 3
round: '3'
country: Polonia
modalita: individuale
quesito: '6'
summary: >-
  Convex hexagon ABCDEF with angle A+C+E=360 deg and (AB/BC)(CD/DE)(EF/FA)=1;
  prove (AB/BF)(FD/DE)(EC/CA)=1.
qa_score: '4'
topics:
  - topic_geometria_piana
methods:
  - method_trigonometria
  - method_simmetria
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1999
  - livello/olimpiade-polacca-round-3
  - round/3
  - topic/geometria_piana
  - gara/individuale
---

*Convex hexagon ABCDEF with angle A+C+E=360 deg and (AB/BC)(CD/DE)(EF/FA)=1; prove (AB/BF)(FD/DE)(EC/CA)=1.*

> A convex hexagon $ABCDEF$ satisfies
> $$\angle A + \angle C + \angle E = 360^\circ \quad \text{and} \quad \frac{AB}{BC} \cdot \frac{CD}{DE} \cdot \frac{EF}{FA} = 1.$$
> Prove that
> $$\frac{AB}{BF} \cdot \frac{FD}{DE} \cdot \frac{EC}{CA} = 1.$$

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_trigonometria|Trigonometria]], [[method_simmetria|Simmetria]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1ROAUIuy9-bXiyz6CZTN9mjOsmZlsxMYt/view)

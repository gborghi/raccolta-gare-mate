---
tipo: quesito
quesito_id: quesito_src_obm_2014_n3_f1__Q18
parent: src_obm_2014_n3_f1
competition: Olimpíada Brasileira de Matemática 2014 — Nível 3 — Fase 1
family: obm
year: '2014'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '18'
summary: 'Count ordered positive-integer pairs (a,b) with 2014/(a^2+b^2) an integer'
qa_score: '4'
topics:
  - topic_aritmetica
methods:
  - method_casework
  - method_fattorizzazione
skills:
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2014
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2014 — Nível 3 — Fase 1 — Quesito 18

*Count ordered positive-integer pairs (a,b) with 2014/(a^2+b^2) an integer*

> How many ordered pairs $(a,b)$ of positive integers exist such that $\frac{2014}{a^2+b^2}$ is an integer?
> (A) $1$ (B) $2$ (C) $3$ (D) $4$ (E) $5$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_casework_accurato|Casework accurato]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

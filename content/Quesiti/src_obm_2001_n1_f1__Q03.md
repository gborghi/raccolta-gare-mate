---
tipo: quesito
quesito_id: quesito_src_obm_2001_n1_f1__Q03
parent: src_obm_2001_n1_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 1 — Fase 1
family: obm
year: '2001'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '3'
summary: 'First three-digit number in the sequence 1, 6, 11, … (step 5)'
qa_score: '4'
answer: C
topics:
  - topic_aritmetica
methods:
  - method_congruenze
skills:
  - skill_riconoscimento_pattern
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2001 — Nível 1 — Fase 1 — Quesito 3

*First three-digit number in the sequence 1, 6, 11, … (step 5)*

> Joana writes the sequence of natural numbers $1, 6, 11, \ldots$, where each number, with the exception of the first, equals the previous one plus five. Joana stops when she finds the first three-digit number of the sequence. That number is:
> 
> (A) $100$ \quad (B) $104$ \quad (C) $101$ \quad (D) $103$ \quad (E) $102$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_congruenze|Congruenze]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** C
**Fonte:** apri PDF

---
title: Olimpiade Matematica Polacca 2000 — Round 1 — Quesito 3
tipo: quesito
quesito_id: quesito_src_pol_2000_r1__Q03
parent: src_pol_2000_r1
competition: Olimpiade Matematica Polacca 2000 — Round 1
family: pol
year: '2000'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '3'
summary: 'For positive a,b,c summing to 1, prove a^2+b^2+c^2+2 sqrt(3abc) <= 1.'
qa_score: '4'
topics:
  - topic_disuguaglianze
methods:
  - method_disuguaglianze
skills:
  - skill_manipolazione_algebrica
  - skill_stima
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/2000
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/disuguaglianze
  - gara/individuale
---

*For positive a,b,c summing to 1, prove a^2+b^2+c^2+2 sqrt(3abc) <= 1.*

> The sum of positive numbers $a, b, c$ is $1$. Prove that $a^2 + b^2 + c^2 + 2\sqrt{3abc} \le 1$.

**Topic:** [[topic_disuguaglianze|Disuguaglianze]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/17HmZVCi_i-CS0_tWLtdYXjFkqYi3fXU0/view)

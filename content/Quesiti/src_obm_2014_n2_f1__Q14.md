---
tipo: quesito
quesito_id: quesito_src_obm_2014_n2_f1__Q14
parent: src_obm_2014_n2_f1
competition: Olimpíada Brasileira de Matemática 2014 — Nível 2 — Fase 1
family: obm
year: '2014'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '14'
summary: '5-digit number xy26c divisible by 8, 9, 11; find x'
qa_score: '3'
answer: A
topics:
  - topic_aritmetica
methods:
  - method_congruenze
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2014
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2014 — Nível 2 — Fase 1 — Quesito 14

*5-digit number xy26c divisible by 8, 9, 11; find x*

> The 5-digit number $\overline{xy26c}$, where each letter represents a digit, is divisible by 8, 9, and 11. What is the value of $x$?
> 
> (A) 3 $\quad$ (B) 5 $\quad$ (C) 1 $\quad$ (D) 4 $\quad$ (E) 9

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_congruenze|Congruenze]], [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** A
**Fonte:** apri PDF

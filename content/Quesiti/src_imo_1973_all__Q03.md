---
title: IMO 1973 (Fifteenth International Olympiad) — Quesito 3
tipo: quesito
quesito_id: quesito_src_imo_1973_all__Q03
parent: src_imo_1973_all
competition: IMO 1973 (Fifteenth International Olympiad)
family: imo
year: '1973'
level: IMO
country: Italia
modalita: individuale
quesito: '3'
summary: Minimize a^2+b^2 for quartic with a real root
topics:
  - topic_algebra
  - topic_disuguaglianze
methods:
  - method_disuguaglianze
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/1973
  - livello/IMO
  - topic/algebra
  - topic/disuguaglianze
  - gara/individuale
---

*Minimize a^2+b^2 for quartic with a real root*

> Siano $a$ e $b$ numeri reali per cui l'equazione
> $$x^4 + ax^3 + bx^2 + ax + 1 = 0$$
> ha almeno una soluzione reale. Per tutte le coppie $(a, b)$ con questa proprietà, trovare il valore minimo di $a^2 + b^2$.

**Topic:** [[topic_algebra|Algebra]], [[topic_disuguaglianze|Disuguaglianze]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze classiche]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF p.1

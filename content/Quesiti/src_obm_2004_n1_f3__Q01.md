---
tipo: quesito
quesito_id: quesito_src_obm_2004_n1_f3__Q01
parent: src_obm_2004_n1_f3
competition: Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 3
family: obm
year: '2004'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '3'
quesito: '1'
summary: Find all 3-digit odd perfect squares whose digit sum is a perfect square
qa_score: '3'
topics:
  - topic_aritmetica
methods:
  - method_casework
  - method_casi_conteggio
skills:
  - skill_conteggio_sistematico
  - skill_lettura_attenta
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-1
  - nivel/1
  - fase/3
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 3 — Quesito 1

*Find all 3-digit odd perfect squares whose digit sum is a perfect square*

> Find all three-digit natural numbers $n$ satisfying all of the following properties:
> \begin{itemize}
> \item $n$ is odd;
> \item $n$ is a perfect square;
> \item the sum of the digits of $n$ is a perfect square.
> \end{itemize}

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]], [[method_casi_conteggio|Casi e conteggio]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_lettura_attenta|Lettura attenta]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

---
title: IMO 1973 (Fifteenth International Olympiad) — Quesito 2
tipo: quesito
quesito_id: quesito_src_imo_1973_all__Q02
parent: src_imo_1973_all
competition: IMO 1973 (Fifteenth International Olympiad)
family: imo
year: '1973'
level: IMO
country: Italia
modalita: individuale
quesito: '2'
summary: Finite non-coplanar set with parallel-pairs property exists?
topics:
  - topic_geometria_solida
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/1973
  - livello/IMO
  - topic/geometria_solida
  - gara/individuale
---

*Finite non-coplanar set with parallel-pairs property exists?*

> Determinare se esiste o meno un insieme finito $M$ di punti nello spazio, non giacenti nello stesso piano, tale che per qualsiasi coppia di punti $A$ e $B$ di $M$ si possano scegliere altri due punti $C$ e $D$ di $M$ in modo che le rette $AB$ e $CD$ siano parallele e non coincidenti.

**Topic:** [[topic_geometria_solida|Geometria solida]]
**Area:** [[Geometria]]
**Fonte:** [apri PDF p.1](https://drive.google.com/file/d/1_yg5ytepW4kPtXzf2MZzjG__z3aJ2dcR/view)

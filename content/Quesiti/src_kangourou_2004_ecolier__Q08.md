---
tipo: quesito
quesito_id: quesito_src_kangourou_2004_ecolier__Q08
parent: src_kangourou_2004_ecolier
competition: Kangourou 2004 Ecolier (18 marzo)
family: kangourou
year: '2004'
level: kangourou
country: Italia
modalita: individuale
quesito: '8'
summary: Quale rettangolo e il negativo
answer: E
topics:
  - topic_geometria_piana
  - topic_combinatoria
skills:
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/italia
  - comp/kangourou
  - anno/2004
  - livello/kangourou
  - topic/geometria_piana
  - topic/combinatoria
  - gara/individuale
---

# Kangourou 2004 Ecolier (18 marzo) — Quesito 8

*Quale rettangolo e il negativo*

![[src_kangourou_2004_ecolier__prob8.png]]

> I rettangoli illustrati hanno tutti le stesse dimensioni. Uno
> solo dei rettangoli A, B, C, D, E è il negativo (eventualmente
> ruotato) di quello a destra (cioè ha quadrati bianchi nelle posizioni in cui quello a destra li ha neri e viceversa). Quale? 
> A)                B)                 C)                  D)                E)
> I quesiti dal N. 9 al N. 16 valgono 4 punti ciascuno

**Topic:** [[topic_geometria_piana|Geometria piana]], [[topic_combinatoria|Combinatoria]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Geometria]]
**Risposta:** E
**Fonte:** apri PDF p.2

---
title: Indian National Mathematical Olympiad 2024 — Quesito 6
tipo: quesito
quesito_id: quesito_src_inmo_2024__Q06
parent: src_inmo_2024
competition: Indian National Mathematical Olympiad 2024
family: inmo
year: '2024'
level: INMO
country: India
modalita: individuale
quesito: '6'
summary: >-
  Sequences A_n and B_n of nested radicals; find positive integers with equal
  floor values
qa_score: '5'
topics:
  - topic_algebra
  - topic_aritmetica
methods:
  - method_estremalita
  - method_telescoping
skills:
  - skill_manipolazione_algebrica
  - skill_stima
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/india
  - comp/inmo
  - anno/2024
  - livello/INMO
  - topic/algebra
  - topic/aritmetica
  - gara/individuale
---

*Sequences A_n and B_n of nested radicals; find positive integers with equal floor values*

> For each positive integer $n \ge 3$, define $A_n$ and $B_n$ as
> $$A_n = \sqrt{n^2+1} + \sqrt{n^2+2} + \cdots + \sqrt{n^2+2n-1},$$
> $$B_n = \sqrt{n^2+2} + \sqrt{n^2+4} + \cdots + \sqrt{n^2+2n}.$$
> Determine all positive integers $n \ge 3$ for which $\lfloor A_n \rfloor = \lfloor B_n \rfloor$.
> 
> \textit{Note.} For any real number $x$, $\lfloor x \rfloor$ denotes the largest integer $N$ such that $N \le x$.

**Topic:** [[topic_algebra|Algebra]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_estremalita|Estremalità]], [[method_telescoping|Telescoping]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]], [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

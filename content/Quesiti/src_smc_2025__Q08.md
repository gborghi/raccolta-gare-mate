---
title: Senior Mathematical Challenge 2025 — Quesito 8
tipo: quesito
quesito_id: quesito_src_smc_2025__Q08
parent: src_smc_2025
competition: Senior Mathematical Challenge 2025
family: smc
year: '2025'
level: Senior Mathematical Challenge
country: Regno Unito
modalita: scelta multipla
quesito: '8'
summary: >-
  Area of shape formed by three arcs of circles of radius 1; P is centre of arc
  BQ and Q is centre of arc PR.
qa_score: '4'
answer: B
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/smc
  - anno/2025
  - livello/senior-mathematical-challenge
  - modalita/scelta-multipla
  - topic/geometria_piana
  - gara/individuale
---

*Area of shape formed by three arcs of circles of radius 1; P is centre of arc BQ and Q is centre of arc PR.*

> The diagram shows three arcs of circles of radius 1. $P$ is the centre of the circle of which $BQ$ is an arc, and $Q$ is the centre of the circle of which $PR$ is an arc. What is the area of this shape?
>
> - **(A)** \frac{\sqrt{3}\pi}{10}
> - **(B)** \frac{\pi}{6}
> - **(C)** \frac{\sqrt{3}\pi}{8}
> - **(D)** \frac{\sqrt{3}}{4}
> - **(E)** \frac{\sqrt{2}\pi}{9}

![[src_smc_2025__Q08.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Risposta:** (B)
**Fonte:** apri PDF

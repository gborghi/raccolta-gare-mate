---
tipo: quesito
quesito_id: quesito_src_obm_2011_n1_f1__Q18
parent: src_obm_2011_n1_f1
competition: Olimpíada Brasileira de Matemática 2011 — Nível 1 — Fase 1
family: obm
year: '2011'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '18'
summary: Sum of numbers on three cube faces in contact with the table
qa_score: '2'
answer: C
topics:
  - topic_geometria_solida
  - topic_logica
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2011
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_solida
  - topic/logica
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2011 — Nível 1 — Fase 1 — Quesito 18

*Sum of numbers on three cube faces in contact with the table*

> In the drawing, three equal cubes resting on a table have their faces painted with the numbers $0, 1, 3, 4, 5$ and $9$. What is the sum of the numbers on all the faces in contact with the table?
> 
> (A) 6 \quad (B) 8 \quad (C) 9 \quad (D) 10 \quad (E) 12

![[src_obm_2011_n1_f1__Q18.png]]

**Topic:** [[topic_geometria_solida|Geometria solida]], [[topic_logica|Logica]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Geometria]], [[Combinatoria, Logica e Probabilita]]
**Risposta:** C
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

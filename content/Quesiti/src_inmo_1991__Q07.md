---
title: Indian National Mathematical Olympiad 1991 — Quesito 7
tipo: quesito
quesito_id: quesito_src_inmo_1991__Q07
parent: src_inmo_1991
competition: Indian National Mathematical Olympiad 1991
family: inmo
year: '1991'
level: INMO
country: India
modalita: individuale
quesito: '7'
summary: Find all real solutions to a symmetric system of three equations
qa_score: '3'
topics:
  - topic_algebra
methods:
  - method_casework
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/india
  - comp/inmo
  - anno/1991
  - livello/INMO
  - topic/algebra
  - gara/individuale
---

*Find all real solutions to a symmetric system of three equations*

> Determine all real solutions $x, y, z$ of the system
> $$\begin{cases} x + y - z = 4, \\ x^2 - y^2 + z^2 = -4, \\ xyz = 6. \end{cases}$$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Casework]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

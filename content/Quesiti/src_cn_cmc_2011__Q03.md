---
title: China Mathematical Competition 2011 — Quesito 3
tipo: quesito
quesito_id: quesito_src_cn_cmc_2011__Q03
parent: src_cn_cmc_2011
competition: China Mathematical Competition 2011
family: cmc
year: '2011'
level: China Mathematical Competition
country: Cina
modalita: individuale
quesito: '3'
summary: 'Find log_a(b) given conditions on positive reals a,b'
qa_score: '3'
answer: '-1'
topics:
  - topic_algebra
  - topic_disuguaglianze
methods:
  - method_disuguaglianze
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/cina
  - comp/cmc
  - anno/2011
  - livello/China-Mathematical-Competition
  - topic/algebra
  - topic/disuguaglianze
  - gara/individuale
---

*Find log_a(b) given conditions on positive reals a,b*

> Suppose $a$ and $b$ are positive real numbers satisfying $\dfrac{1}{a} + \dfrac{1}{b} \leq 2\sqrt{2}$ and $(a-b)^2 = 4(ab)^3$. Then $\log_a b =$ ________.

**Topic:** [[topic_algebra|Algebra]], [[topic_disuguaglianze|Disuguaglianze]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]], [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]]
**Risposta:** -1
**Fonte:** apri libro PDF

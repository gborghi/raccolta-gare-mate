...
quesito: '6'
summary: 'Find all f:R→R satisfying f(x+y)=f(x)f(y)f(xy)'
qa_score: '3'
topics:
...
> Let $\mathbf{R}$ denote the set of real numbers. Find all functions $f : \mathbf{R} \to \mathbf{R}$ satisfying the condition
> $$f(x + y) = f(x)f(y)f(xy)$$

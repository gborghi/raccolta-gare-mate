---
title: IMO 2023 (LXIV International Mathematical Olympiad) — Quesito 1
tipo: quesito
quesito_id: quesito_src_imho_2023__Q01
parent: src_imho_2023
competition: IMO 2023 (LXIV International Mathematical Olympiad)
family: imo
year: '2023'
level: IMO
country: Italia
modalita: individuale
quesito: '1'
summary: Composite integers n>1 with divisibility condition on divisors
qa_score: '3'
topics:
  - topic_aritmetica
methods:
  - method_casework
  - method_fattorizzazione
skills:
  - skill_lettura_attenta
  - skill_manipolazione_algebrica
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/2023
  - livello/IMO
  - topic/aritmetica
  - gara/individuale
---

*Composite integers n>1 with divisibility condition on divisors*

> Determine all composite integers $n > 1$ that satisfy the following property: if $d_1, d_2, \ldots, d_k$ are all the positive divisors of $n$ with $1 = d_1 < d_2 < \cdots < d_k = n$, then $d_i$ divides $d_{i+1} + d_{i+2}$ for every $1 \le i \le k - 2$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** [apri PDF p.1](https://drive.google.com/file/d/19SQ1XZQ_GEiLN_l4y9fIOBkuxg2liNyN/view)

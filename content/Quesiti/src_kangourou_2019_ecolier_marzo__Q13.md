---
title: Kangourou 2019 Gara di Marzo Ecolier — Quesito 13
tipo: quesito
quesito_id: quesito_src_kangourou_2019_ecolier_marzo__Q13
parent: src_kangourou_2019_ecolier_marzo
competition: Kangourou 2019 Gara di Marzo Ecolier
family: kangourou
year: '2019'
level: kangourou
country: Italia
modalita: individuale
quesito: '13'
summary: 'Cifra nella casella ? per somma massima con 2,0,1,9'
answer: A
topics:
  - topic_logica
  - topic_aritmetica
methods:
  - method_casework
skills:
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/italia
  - comp/kangourou
  - anno/2019
  - livello/kangourou
  - topic/logica
  - topic/aritmetica
  - gara/individuale
---

*Cifra nella casella ? per somma massima con 2,0,1,9*

![[src_kangourou_2019_ecolier_marzo__prob13.png]]

> Stefano ha a disposizione le cifre 2, 0, 1 e 9. In 
> ognuna delle caselle in figura ne deve inserire una e 
> le deve usare tutte. Se vuole che la somma indicata 
> fornisca il risultato più grande possibile, che cifra dovrà inserire nella casella indicata 
> con il punto di domanda? 
> A) 0 oppure 1.	
> 	
> B) 0 oppure 2.	
> 	
> C) Necessariamente 0.  
> D) Necessariamente 1.	
> E) Necessariamente 2.

**Topic:** [[topic_logica|Logica, giochi, strategie]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Analisi per casi]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Combinatoria, Logica e Probabilita]]
**Risposta:** A
**Fonte:** apri PDF p.3

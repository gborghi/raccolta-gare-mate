---
tipo: quesito
quesito_id: quesito_src_obm_2001_n2_f1__Q09
parent: src_obm_2001_n2_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1
family: obm
year: '2001'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '9'
summary: Rectangle with distinct integer sides whose perimeter equals area (annulled)
qa_score: '2'
answer: Anulada
topics:
  - topic_algebra
  - topic_aritmetica
methods:
  - method_fattorizzazione
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/algebra
  - topic/aritmetica
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1 — Quesito 9

*Rectangle with distinct integer sides whose perimeter equals area (annulled)*

> The measures of the sides of a rectangle are distinct integers. The lengths of these sides, such that the perimeter and the area of the rectangle are expressed by the same number, are:
> 
> (A) $18$ \quad (B) $12$ \quad (C) $24$ \quad (D) $9$ \quad (E) $36$

**Topic:** [[topic_algebra|Algebra]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Algebra e Analisi]], [[Aritmetica e Teoria dei Numeri]]
**Risposta:** Anulada
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

---
tipo: quesito
quesito_id: quesito_src_jmo15yq_yosen__Q06
parent: src_jmo15yq_yosen
competition: Japan Mathematical Olympiad 2005 — Yosen (Preliminary)
family: jmo
year: '2005'
level: JMO Yosen
country: Giappone
modalita: individuale
quesito: '6'
summary: 'Minimum of 2^a + 4^b for real a,b with a+b=17'
qa_score: '3'
answer: '2^{35/3}'
topics:
  - topic_algebra
  - topic_disuguaglianze
methods:
  - method_disuguaglianze
  - method_estremalita
skills:
  - skill_manipolazione_algebrica
  - skill_stima
tags:
  - kg/quesito
  - paese/giappone
  - comp/jmo
  - anno/2005
  - livello/JMO-Yosen
  - topic/algebra
  - topic/disuguaglianze
  - gara/individuale
---

# Japan Mathematical Olympiad 2005 — Yosen (Preliminary) — Quesito 6

*Minimum of 2^a + 4^b for real a,b with a+b=17*

> For real numbers $a, b$ satisfying $a + b = 17$, find the minimum value of $2^a + 4^b$.

**Topic:** [[topic_algebra|Algebra]], [[topic_disuguaglianze|Disuguaglianze]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]]
**Area:** [[Algebra e Analisi]]
**Risposta:** $2^{35/3}$
**Fonte:** apri PDF

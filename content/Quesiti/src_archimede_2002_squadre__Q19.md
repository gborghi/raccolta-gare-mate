---
title: Archimede 2002 - Gara Nazionale a Squadre (Cesenatico) — Quesito 19
tipo: quesito
quesito_id: quesito_src_archimede_2002_squadre__Q19
parent: src_archimede_2002_squadre
competition: Archimede 2002 - Gara Nazionale a Squadre (Cesenatico)
family: archimede
year: '2002'
level: squadre
country: Italia
modalita: squadre
quesito: '19'
summary: area dello stemma
answer: 0119
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/italia
  - comp/archimede
  - anno/2002
  - livello/squadre
  - topic/geometria_piana
  - gara/squadre
---

*area dello stemma*

> **Stemmi molto geometrici** — 70 punti
> 
> Lo stemma dell'università di Matelandia è costituito di 2 cerchi tangenti internamente in un punto $P$. Un membro si è divertito a tracciare sullo stemma della porta del rettorato una corda $AB$ del cerchio più grande tangente al cerchio più piccolo e a tracciare le due linee che congiungono $A$ e $B$ con $P$. Chiamiamo $D$ ed $E$ le intersezioni, rispettivamente, di $AP$ e $BP$ col cerchio più piccolo. Sapendo che $AB = 196$, $PD = 17$ e $PE = 11$ calcolare $AC$.

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Geometria]]
**Risposta:** 0119
**Fonte:** [apri PDF p.3](https://drive.google.com/file/d/16VSh_6MxR-WoexE5tqBJtT-NsZAD5ImX/view)

---
title: Gara a Squadre 2009 — Quesito 9
tipo: quesito
quesito_id: quesito_src_bocconi_squadre_2009__Q09
parent: src_bocconi_squadre_2009
competition: Gara a Squadre 2009
family: bocconi
year: '2009'
level: Gara a Squadre
country: Italia
modalita: a squadre
quesito: '9'
summary: Lunghezza spigolo AE di piramide a base rettangolare
qa_score: '4'
answer: '$AE = 60\text{ m}$'
topics:
  - topic_geometria_solida
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/italia
  - comp/gara-squadre
  - anno/2009
  - livello/gara-squadre
  - topic/geometria_solida
  - gara/squadre
---

*Lunghezza spigolo AE di piramide a base rettangolare*

> Abbiamo misurato gli spigoli della piramide a base rettangolare che vedete in figura (che pure non rispetta le proporzioni): $AB = 90\text{ m}$; $AC = 70\text{ m}$; $AD = 20\text{ m}$. Poi ci siamo stancati. Quanto misura lo spigolo $AE$?

![[src_bocconi_squadre_2009__Q09.png]]

**Topic:** [[topic_geometria_solida|Geometria solida]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Risposta:** $AE = 60\text{ m}$
**Fonte:** [apri PDF](https://drive.google.com/file/d/1kquAXdz_1_nXRu6LOSGozuEvU4vIuFnH/view)

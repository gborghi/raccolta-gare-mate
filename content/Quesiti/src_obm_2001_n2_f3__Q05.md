---
tipo: quesito
quesito_id: quesito_src_obm_2001_n2_f3__Q05
parent: src_obm_2001_n2_f3
competition: Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 3
family: obm
year: '2001'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '3'
quesito: '5'
summary: 'Find all possible values of a symmetric expression in a, b, c given a+b+c=0'
qa_score: '4'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_simmetria
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-2
  - nivel/2
  - fase/3
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 3 — Quesito 5

*Find all possible values of a symmetric expression in a, b, c given a+b+c=0*

> Let $a$, $b$ and $c$ be nonzero real numbers such that $a+b+c=0$. Compute the possible values of
> $$\frac{(a^3+b^3+c^3)^2(a^4+b^4+c^4)}{(a^5+b^5+c^5)^2}.$$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_simmetria|Simmetria]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

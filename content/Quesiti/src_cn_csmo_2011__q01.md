---
title: China Southeastern Mathematical Olympiad 2011 — Quesito 1
tipo: quesito
lang: en
quesito_id: quesito_src_cn_csmo_2011__Q01
parent: src_cn_csmo_2011
competition: China Southeastern Mathematical Olympiad 2011
family: csmo
year: '2011'
level: China Southeastern Mathematical Olympiad
country: Cina
modalita: individuale
quesito: '1'
summary: Find range of b and value of a given a minimum of a rational function equals 3
qa_score: '3'
topics:
  - topic_algebra
  - topic_insiemi_funzioni
methods:
  - method_disuguaglianze
  - method_estremalita
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
  - skill_stima
tags:
  - kg/quesito
  - paese/cina
  - comp/csmo
  - anno/2011
  - livello/China-Southeastern-Mathematical-Olympiad
  - topic/algebra
  - topic/insiemi_funzioni
  - gara/individuale
---
<div class="qlang-switch" data-default="en"></div>


*Find range of b and value of a given a minimum of a rational function equals 3*

> If $\displaystyle\min_{x\in\mathbf{R}} \frac{ax^2+b}{\sqrt{x^2+1}} = 3$, find
> 
> (1) the range of $b$;
> 
> (2) the value of $a$ for given $b$. (posed by Lu Xingjiang)

**Topic:** [[topic_algebra|Algebra]], [[topic_insiemi_funzioni|Insiemi e funzioni]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]], [[skill_stima|Stima]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri libro PDF](https://drive.google.com/file/d/1cjmD7lTT4vb3nB3ylcVCHeHucUE2rNI0/view)


<span class="qlang-split" data-lang="it"></span>


*Ricerca l'intervallo di b e il valore di un dato minimo di una funzione razionale pari a 3*

> Se < MSK0/>, trovare
> 
> (1) l'intervallo di $b$;
> 
> (2) il valore di $a$ per un dato $b$. (Posizionato da Lu Xingjiang)

[[src_cn_csmo_2011__Q01]]

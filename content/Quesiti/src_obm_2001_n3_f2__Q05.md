---
tipo: quesito
quesito_id: quesito_src_obm_2001_n3_f2__Q05
parent: src_obm_2001_n3_f2
competition: Olimpíada Brasileira de Matemática 2001 — Nível 3 — Fase 2
family: obm
year: '2001'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '2'
quesito: '5'
summary: Evaluate nested radical sum expression and express as rational p/q
qa_score: '4'
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_telescoping
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-3
  - nivel/3
  - fase/2
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2001 — Nível 3 — Fase 2 — Quesito 5

*Evaluate nested radical sum expression and express as rational p/q*

> The number
> $$S = \sum_{a=1}^{2000} \sqrt{1 + \frac{1}{a^2} + \frac{1}{(a+1)^2}}$$
> is rational; express it in the form $\dfrac{p}{q}$, where $p$ and $q$ are positive integers.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_telescoping|Telescoping]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Fonte:** apri PDF

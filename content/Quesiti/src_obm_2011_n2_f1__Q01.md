---
tipo: quesito
quesito_id: quesito_src_obm_2011_n2_f1__Q01
parent: src_obm_2011_n2_f1
competition: Olimpíada Brasileira de Matemática 2011 — Nível 2 — Fase 1
family: obm
year: '2011'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '1'
summary: Minimum toll on one-way road map from A to B
qa_score: '3'
topics:
  - topic_combinatoria
methods:
  - method_estremalita
  - method_casework
skills:
  - skill_modellizzazione
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2011
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/combinatoria
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2011 — Nível 2 — Fase 1 — Quesito 1

*Minimum toll on one-way road map from A to B*

> The figure beside shows a road map. The numbers written next to the arrows indicate how much one pays, in coins, to travel along that road. All the roads are one-way, in the direction indicated by the arrows. What is the minimum value of the toll one pays leaving city $A$ and arriving at city $B$?
> 
> (A) $11$ \quad (B) $14$ \quad (C) $12$ \quad (D) $10$ \quad (E) $15$

![[src_obm_2011_n2_f1__Q01.png]]

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_estremalita|Estremalità]], [[method_casework|Casework]]
**Abilita:** [[skill_modellizzazione|Modellizzazione]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri PDF

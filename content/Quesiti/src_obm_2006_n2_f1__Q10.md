---
title: Olimpíada Brasileira de Matemática 2006 — Nível 2 — Fase 1 — Quesito 10
tipo: quesito
quesito_id: quesito_src_obm_2006_n2_f1__Q10
parent: src_obm_2006_n2_f1
competition: Olimpíada Brasileira de Matemática 2006 — Nível 2 — Fase 1
family: obm
year: '2006'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '10'
summary: Count squares constructible from dot grid
qa_score: '3'
answer: A
topics:
  - topic_combinatoria
  - topic_geometria_piana
methods:
  - method_conteggio
  - method_casework
skills:
  - skill_conteggio_sistematico
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2006
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/combinatoria
  - topic/geometria_piana
  - gara/individuale
---

*Count squares constructible from dot grid*

> The number of squares that can be constructed with vertices on the points of the figure below (a $4\times 4$ array of dots arranged so that each row has 4 dots and there are 4 rows):
> 
> **A)** 18 \quad **B)** 14 \quad **C)** 9 \quad **D)** 20 \quad **E)** 10

![[src_obm_2006_n2_f1__Q10.png]]

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_conteggio|Conteggio]], [[method_casework|Casework]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Geometria]]
**Risposta:** A
**Fonte:** [apri PDF](https://drive.google.com/file/d/14t6nVajpMlWqHRaEJhIgWxeB33efd9t2/view)

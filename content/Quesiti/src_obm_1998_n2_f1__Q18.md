---
tipo: quesito
quesito_id: quesito_src_obm_1998_n2_f1__Q18
parent: src_obm_1998_n2_f1
competition: Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1
family: obm
year: '1998'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '18'
summary: 'Count ways to paint flag with 4 colours, two cannot be adjacent'
qa_score: '3'
answer: A
topics:
  - topic_combinatoria
methods:
  - method_colorazione
  - method_casework
  - method_conteggio
skills:
  - skill_conteggio_sistematico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1998
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/combinatoria
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1 — Quesito 18

*Count ways to paint flag with 4 colours, two cannot be adjacent*

> You are going to paint the flag below using 4 colours: blue, green, yellow, and red, one colour per region.
> 
> If red and yellow cannot be adjacent, in how many ways can the flag be painted?
> 
> A) 12
> B) 4
> C) 18
> D) 20
> E) 16

![[src_obm_1998_n2_f1__Q18.png]]

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_colorazione|Colorazione]], [[method_casework|Casework]], [[method_conteggio|Conteggio]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** A
**Fonte:** apri PDF

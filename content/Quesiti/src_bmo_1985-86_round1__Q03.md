---
tipo: quesito
quesito_id: quesito_src_bmo_1985-86_round1__Q03
parent: src_bmo_1985-86_round1
competition: British Mathematical Olympiad 1985-86 — Round 1
family: bmo
year: '1985'
level: BMO Round 1
country: Regno Unito
modalita: individuale
quesito: '3'
summary: Prove a radical equation in positive integers forces m to be a square
qa_score: '3'
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_induzione
  - method_congruenze
skills:
  - skill_manipolazione_algebrica
  - skill_astrazione
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/bmo
  - anno/1985
  - livello/BMO-Round-1
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# British Mathematical Olympiad 1985-86 — Round 1 — Quesito 3

*Prove a radical equation in positive integers forces m to be a square*

> Prove that if $m$, $n$ are positive integers and
> $$1 + m + n\sqrt{3} = \left(2 + \sqrt{3}\right)^{2n-1}$$
> then $m$ is a perfect square.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_induzione|Induzione]], [[method_congruenze|Congruenze]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_astrazione|Astrazione]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Fonte:** apri PDF

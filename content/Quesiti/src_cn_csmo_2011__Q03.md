---
title: China Southeastern Mathematical Olympiad 2011 — Quesito 3
tipo: quesito
quesito_id: quesito_src_cn_csmo_2011__Q03
parent: src_cn_csmo_2011
competition: China Southeastern Mathematical Olympiad 2011
family: csmo
year: '2011'
level: China Southeastern Mathematical Olympiad
country: Cina
modalita: individuale
quesito: '3'
summary: >-
  Find all n so every 35-element subset of {1,...,50} contains a,b with a+b=n or
  a-b=n
qa_score: '3'
topics:
  - topic_combinatoria
methods:
  - method_casework
  - method_cassetti
skills:
  - skill_conteggio_sistematico
  - skill_casework_accurato
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/cina
  - comp/csmo
  - anno/2011
  - livello/China-Southeastern-Mathematical-Olympiad
  - topic/combinatoria
  - gara/individuale
---

*Find all n so every 35-element subset of {1,...,50} contains a,b with a+b=n or a-b=n*

> Let set $M = \{1, 2, 3, \ldots, 50\}$. Find all positive integers $n$ such that there are at least two different elements $a$ and $b$ in any subset with 35 elements of $M$, such that $a + b = n$ or $a - b = n$. (posed by Li Shenghong)

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_casework|Casework]], [[method_cassetti|Principio dei cassetti]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_casework_accurato|Casework accurato]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri libro PDF

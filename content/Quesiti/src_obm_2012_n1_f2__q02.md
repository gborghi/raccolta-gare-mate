---
title: Olimpíada Brasileira de Matemática 2012 — Nível 1 — Fase 2 — Quesito 2
tipo: quesito
quesito_id: quesito_src_obm_2012_n1_f2__Q02
parent: src_obm_2012_n1_f2
competition: Olimpíada Brasileira de Matemática 2012 — Nível 1 — Fase 2
family: obm
year: '2012'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '2'
quesito: '2'
summary: Find ZYX from a cryptarithmetic column addition
qa_score: '3'
topics:
  - topic_aritmetica
methods:
  - method_casework
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2012
  - livello/OBM-Nível-1
  - nivel/1
  - fase/2
  - topic/aritmetica
  - gara/individuale
---

*Find ZYX from a cryptarithmetic column addition*

> In the addition of three 4-digit numbers below, different letters represent different digits. What is the number $\overline{ZYX}$?
> $$\begin{array}{r} XXXX \\ +\; YYYY \\ \hline ZZZZ \\ \hline YXXZ \end{array}$$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]], [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1GopwL_0fgb7iEbs-bG1HNdfWHmDyf8B2/view)

---
tipo: quesito
quesito_id: quesito_src_obm_2005_n1_f1__Q20
parent: src_obm_2005_n1_f1
competition: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1
family: obm
year: '2005'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '20'
summary: Minimum colors to paint 3x3 grid with no row/column/diagonal repeat
qa_score: '3'
answer: B
topics:
  - topic_combinatoria
  - topic_logica
methods:
  - method_colorazione
  - method_casework
skills:
  - skill_conteggio_sistematico
  - skill_ragionamento_geometrico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2005
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/combinatoria
  - topic/logica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1 — Quesito 20

*Minimum colors to paint 3x3 grid with no row/column/diagonal repeat*

> The nine cells of a $3 \times 3$ board must be painted so that no two cells of the same colour appear in the same column, the same row, or either of the two main diagonals. What is the minimum number of colours needed?
> 
> A) $3$ \quad B) $4$ \quad C) $5$ \quad D) $6$ \quad E) $7$

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_logica|Logica]]
**Metodo:** [[method_colorazione|Colorazione]], [[method_casework|Casework]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** B
**Fonte:** apri PDF

---
title: Japan Junior Mathematical Olympiad 2003 — Quesito 2
tipo: quesito
quesito_id: quesito_src_jjmo1q__Q02
parent: src_jjmo1q
competition: Japan Junior Mathematical Olympiad 2003
family: jjmo
year: '2003'
level: JJMO
country: Giappone
modalita: individuale
quesito: '2'
summary: 'Solve a symmetric system of three linear equations in x, y, z'
qa_score: '3'
topics:
  - topic_algebra
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/giappone
  - comp/jjmo
  - anno/2003
  - livello/JJMO
  - topic/algebra
  - gara/individuale
---

*Solve a symmetric system of three linear equations in x, y, z*

> Find all real numbers $x, y, z$ that simultaneously satisfy all three of the following equations:
> $$\begin{cases} y + z = 3 \\ x + z = 5 \\ x + y = 4 \end{cases}$$

**Topic:** [[topic_algebra|Algebra]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/16MD0CR3222jwePcS-UGcyoCO-ihqCRNA/view)

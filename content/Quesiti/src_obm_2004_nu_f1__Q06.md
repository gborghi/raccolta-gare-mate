---
tipo: quesito
quesito_id: quesito_src_obm_2004_nu_f1__Q06
parent: src_obm_2004_nu_f1
competition: Olimpíada Brasileira de Matemática 2004 — Nível Universitário — Fase 1
family: obm
year: '2004'
level: OBM Nível Universitário
country: Brasile
modalita: individuale
nivel: u
fase: '1'
quesito: '6'
summary: Compute a telescoping sum involving cubic-like factors
qa_score: '4'
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_telescoping
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-Universitário
  - nivel/u
  - fase/1
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2004 — Nível Universitário — Fase 1 — Quesito 6

*Compute a telescoping sum involving cubic-like factors*

> Compute $\displaystyle\sum_{k=1}^{\infty} \frac{1}{(3k+1)(3k+2)(3k+3)}$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_telescoping|Telescoping]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Fonte:** apri PDF

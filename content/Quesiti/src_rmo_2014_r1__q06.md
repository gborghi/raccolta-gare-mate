---
title: Regional Mathematical Olympiad 2014 — Region 1 — Quesito 6
tipo: quesito
quesito_id: quesito_src_rmo_2014_r1__Q06
parent: src_rmo_2014_r1
competition: Regional Mathematical Olympiad 2014
family: rmo
year: '2014'
level: RMO
country: India
modalita: individuale
quesito: '6'
summary: Positive reals summing to 1; find smallest K for sum inequality
regioni:
  - Region 1
qa_score: '4'
topics:
  - topic_disuguaglianze
  - topic_algebra
methods:
  - method_disuguaglianze
  - method_estremalita
skills:
  - skill_manipolazione_algebrica
  - skill_stima
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/india
  - comp/rmo
  - anno/2014
  - livello/RMO
  - topic/disuguaglianze
  - topic/algebra
  - regione/r1
  - gara/individuale
---

*Positive reals summing to 1; find smallest K for sum inequality*

> Let $x_1, x_2, \ldots, x_{2014}$ be positive real numbers such that $\sum_{j=1}^{2014} x_j = 1$. Determine with proof the smallest constant $K$ such that
> $$K \sum_{j=1}^{2014} \frac{x_j^2}{1 - x_j} \ge 1.$$

**Topic:** [[topic_disuguaglianze|Disuguaglianze]], [[topic_algebra|Algebra]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1gebds3GWDOcaRUuyUCK_TW2oDlckyr2-/view)

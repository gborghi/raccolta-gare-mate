---
tipo: quesito
quesito_id: quesito_src_jmo25yq_yosen__Q06
parent: src_jmo25yq_yosen
competition: Japan Mathematical Olympiad 2015 — Yosen (Preliminary)
family: jmo
year: '2015'
level: JMO Yosen
country: Giappone
modalita: individuale
quesito: '6'
summary: Find minimum a+b+c for positive integers satisfying four GCD conditions
qa_score: '4'
answer: '30'
topics:
  - topic_aritmetica
methods:
  - method_estremalita
  - method_casework
skills:
  - skill_lettura_attenta
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/giappone
  - comp/jmo
  - anno/2015
  - livello/JMO-Yosen
  - topic/aritmetica
  - gara/individuale
---

# Japan Mathematical Olympiad 2015 — Yosen (Preliminary) — Quesito 6

*Find minimum a+b+c for positive integers satisfying four GCD conditions*

> Positive integers $a, b, c$ satisfy all four of the following conditions:
> \begin{itemize}
> \item $\gcd(a, b, c) = 1$,
> \item $\gcd(a, b) + c > 1$,
> \item $\gcd(b, c) + a > 1$,
> \item $\gcd(c, a) + b > 1$.
> \end{itemize}
> Find the minimum value of $a + b + c$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_estremalita|Estremalità]], [[method_casework|Casework]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** 30
**Fonte:** apri PDF

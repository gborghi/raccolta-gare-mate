---
title: Olimpiade Matematica Polacca 1998 — Round 1 — Quesito 2
tipo: quesito
quesito_id: quesito_src_pol_1998_r1__Q02
parent: src_pol_1998_r1
competition: Olimpiade Matematica Polacca 1998 — Round 1
family: pol
year: '1998'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '2'
summary: Orthocenter and circumcenter equidistant from a vertex; find angle CAB
qa_score: '4'
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1998
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/geometria_piana
  - gara/individuale
---

*Orthocenter and circumcenter equidistant from a vertex; find angle CAB*

> Let $H$ be the orthocenter of a triangle inscribed in a circle with center $O$. Given that $AO = AH$, find the measure of $\angle CAB$.

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Geometria]]
**Fonte:** apri PDF

---
title: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1 — Quesito 2
tipo: quesito
quesito_id: quesito_src_obm_2005_n1_f1__Q02
parent: src_obm_2005_n1_f1
competition: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1
family: obm
year: '2005'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '2'
summary: Deducing color of removed marbles from a box
qa_score: '3'
answer: E
topics:
  - topic_logica
  - topic_combinatoria
methods:
  - method_casework
skills:
  - skill_lettura_attenta
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2005
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/logica
  - topic/combinatoria
  - gara/individuale
---

*Deducing color of removed marbles from a box*

> A box contains 3 red marbles, 2 white marbles, and 1 black marble. Professor Piraldo removed 3 marbles from the box. Knowing that none of the removed marbles is black, we can say about the 3 removed marbles that:
> 
> A) they are all the same color.
> B) they are all red.
> C) one is red and two are white.
> D) one is white and two are red.
> E) at least one is red.

**Topic:** [[topic_logica|Logica]], [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** E
**Fonte:** apri PDF

---
tipo: quesito
quesito_id: quesito_src_obm_2001_n2_f1__Q18
parent: src_obm_2001_n2_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1
family: obm
year: '2001'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '18'
summary: Sum of all numbers from 1 to 999 in which the digit 1 appears exactly twice
qa_score: '2'
answer: A
topics:
  - topic_combinatoria
  - topic_aritmetica
methods:
  - method_casework
  - method_conteggio
skills:
  - skill_conteggio_sistematico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/combinatoria
  - topic/aritmetica
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2001 — Nível 2 — Fase 1 — Quesito 18

*Sum of all numbers from 1 to 999 in which the digit 1 appears exactly twice*

> All the numbers from $1$ to $999$ in which the digit $1$ appears exactly $2$ times (such as $11$, $121$, $411$, etc.) are written down. The sum of all these numbers is:
> 
> (A) $6882$ \quad (B) $5994$ \quad (C) $4668$ \quad (D) $7224$ \quad (E) $3448$

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]], [[method_conteggio|Conteggio]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Aritmetica e Teoria dei Numeri]]
**Risposta:** A
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

---
tipo: quesito
quesito_id: quesito_src_ioqm_2021__Q10
parent: src_ioqm_2021
competition: Indian Olympiad Qualifier in Mathematics (IOQM) 2021
family: ioqm
year: '2021'
level: IOQM
country: India
modalita: individuale
quesito: '10'
summary: >-
  Polynomial of least degree with integer coefficients satisfying
  P(sqrt(7)+sqrt(5))=2(sqrt(7)-sqrt(5)); find P(2)
qa_score: '3'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_manipolazione_algebrica
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/india
  - comp/ioqm
  - anno/2021
  - livello/IOQM
  - topic/algebra
  - gara/individuale
---

# Indian Olympiad Qualifier in Mathematics (IOQM) 2021 — Quesito 10

*Polynomial of least degree with integer coefficients satisfying P(sqrt(7)+sqrt(5))=2(sqrt(7)-sqrt(5)); find P(2)*

> Suppose $P$ is the polynomial of least degree with integer coefficients such that
> $$P(\sqrt{7} + \sqrt{5}) = 2(\sqrt{7} - \sqrt{5})$$
> Find $P(2)$.

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_manipolazione_algebrica|method_manipolazione_algebrica]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

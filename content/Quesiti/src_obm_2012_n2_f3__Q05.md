---
tipo: quesito
quesito_id: quesito_src_obm_2012_n2_f3__Q05
parent: src_obm_2012_n2_f3
competition: Olimpíada Brasileira de Matemática 2012 — Nível 2 — Fase 3
family: obm
year: '2012'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '3'
quesito: '5'
summary: Find a+b given two equations in real numbers a and b
qa_score: '3'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_simmetria
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2012
  - livello/OBM-Nível-2
  - nivel/2
  - fase/3
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2012 — Nível 2 — Fase 3 — Quesito 5

*Find a+b given two equations in real numbers a and b*

> Consider the real numbers $a$ and $b$ such that $(a + b)(a + 1)(b + 1) = 2$ and $a^2 + b^2 = 1$. Find the value of $a + b$.

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_simmetria|Simmetria]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

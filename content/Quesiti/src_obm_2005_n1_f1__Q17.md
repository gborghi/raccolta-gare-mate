---
tipo: quesito
quesito_id: quesito_src_obm_2005_n1_f1__Q17
parent: src_obm_2005_n1_f1
competition: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1
family: obm
year: '2005'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '17'
summary: Number of squares needed to balance the last scale
qa_score: '3'
answer: C
topics:
  - topic_algebra
  - topic_logica
skills:
  - skill_modellizzazione
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2005
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/algebra
  - topic/logica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1 — Quesito 17

*Number of squares needed to balance the last scale*

> Figures of the same shape represent objects of the same mass. How many squares are needed to balance the last scale?
> 
> A) $7$ \quad B) $8$ \quad C) $9$ \quad D) $10$ \quad E) $12$

![[src_obm_2005_n1_f1__Q17.png]]

**Topic:** [[topic_algebra|Algebra]], [[topic_logica|Logica]]
**Abilita:** [[skill_modellizzazione|Modellizzazione]], [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]], [[Combinatoria, Logica e Probabilita]]
**Risposta:** C
**Fonte:** apri PDF

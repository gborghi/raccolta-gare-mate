---
tipo: quesito
quesito_id: quesito_src_obm_1999_n3_f2__Q04
parent: src_obm_1999_n3_f2
competition: Olimpíada Brasileira de Matemática 1999 — Nível 3 — Fase 2
family: obm
year: '1999'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '2'
quesito: '4'
summary: Positive integers n for which a 9×10 rectangle can be tiled by 1×n pieces
qa_score: '4'
topics:
  - topic_combinatoria
methods:
  - method_casework
  - method_colorazione
  - method_invarianti
skills:
  - skill_ragionamento_geometrico
  - skill_conteggio_sistematico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1999
  - livello/OBM-Nível-3
  - nivel/3
  - fase/2
  - topic/combinatoria
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 1999 — Nível 3 — Fase 2 — Quesito 4

*Positive integers n for which a 9×10 rectangle can be tiled by 1×n pieces*

> Determine all positive integers $n$ for which it is possible to tile a $9 \times 10$ rectangle using $1 \times n$ pieces.

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_casework|Casework]], [[method_colorazione|Colorazione]], [[method_invarianti|Invarianti]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri PDF

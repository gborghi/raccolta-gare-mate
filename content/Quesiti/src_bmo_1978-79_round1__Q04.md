...
quesito: '4'
summary: >-
  Rational-valued function satisfying f(x+f(y))=f(f(x))+f(y); prove f is
  constant
qa_score: '4'
topics:
  - topic_funzionali
...
> The function $f$ is defined on the rational numbers and takes only rational values. For all rational $x$ and $y$,
> $$f(x + f(y)) = f(f(x)) + f(y).$$
> 

---
tipo: quesito
quesito_id: quesito_src_kangourou_2004_benjamin__Q06
parent: src_kangourou_2004_benjamin
competition: Kangourou 2004 Benjamin (18 marzo)
family: kangourou
year: '2004'
level: kangourou
country: Italia
modalita: individuale
quesito: '6'
summary: Indovinare il numero (verita tra amici)
answer: B
topics:
  - topic_logica
  - topic_aritmetica
skills:
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/italia
  - comp/kangourou
  - anno/2004
  - livello/kangourou
  - topic/logica
  - topic/aritmetica
  - gara/individuale
---

# Kangourou 2004 Benjamin (18 marzo) — Quesito 6

*Indovinare il numero (verita tra amici)*

> Devi indovinare un numero intero positivo sul quale quattro tuoi
> amici ti danno le seguenti informazioni. Andrea: "Il numero è 9".
> Bruno: "Il numero è primo". Carlo: "Il numero è pari". Dario: "Il
> numero è 15". Sai che, tra Andrea e Bruno, uno dei due dice la verità
> P
> ? C
> F P
> Q
> C
> 5 cm
> 9-14-Benjamin.qxd  22/02/2004  16.53  Pagina  10
> 
> Pag. 
> Pag. 11
> 11
> Kang 2004
> Kang 2004
> Benjamin
> Benjamin
> e così pure fra Carlo e Dario. Qual è il numero?
> A) 1
> B) 2
> C) 3   
> D) 9           E) 15

**Topic:** [[topic_logica|Logica, giochi, strategie]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Abilita:** [[skill_casework_accurato|Casework accurato]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Combinatoria, Logica e Probabilita]]
**Risposta:** B
**Fonte:** apri PDF p.2

---
title: Olimpíada Brasileira de Matemática 2007 — Nível 3 — Fase 1 — Quesito 17
tipo: quesito
quesito_id: quesito_src_obm_2007_n3_f1__Q17
parent: src_obm_2007_n3_f1
competition: Olimpíada Brasileira de Matemática 2007 — Nível 3 — Fase 1
family: obm
year: '2007'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '17'
summary: Count integer pairs with x^8+3y^4=4x^2 y^3 and 1<=y<=2007
qa_score: '5'
answer: E
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2007
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

*Count integer pairs with x^8+3y^4=4x^2 y^3 and 1<=y<=2007*

> The number of pairs $(x,y)$ of positive integers that satisfy the equation
> $$x^8+3y^4=4x^2y^3,$$
> with $1\le y\le 2007$, is equal to:
> 
> (A) $40$ \quad (B) $41$ \quad (C) $42$ \quad (D) $43$ \quad (E) $44$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Risposta:** E
**Fonte:** [apri PDF](https://drive.google.com/file/d/1phTZyNXq38lPPpmVgAQtu6VYCLZz0aeL/view)

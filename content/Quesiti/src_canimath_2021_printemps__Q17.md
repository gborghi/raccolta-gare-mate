---
title: Coupe Animath Printemps 2021 — Quesito 17
tipo: quesito
quesito_id: quesito_src_canimath_2021_printemps__Q17
parent: src_canimath_2021_printemps
competition: Coupe Animath Printemps 2021
family: coupe_animath
year: '2021'
level: Coupe Animath Printemps
country: Francia
modalita: individuale
stagione: printemps
quesito: '17'
summary: '8 ants on the edges of a unit cube: two at geodesic distance at most 1 (lycee)'
qa_score: '3'
topics:
  - topic_combinatoria
  - topic_geometria_solida
methods:
  - method_cassetti
  - method_estremalita
skills:
  - skill_modellizzazione
  - skill_ragionamento_geometrico
  - skill_stima
tags:
  - kg/quesito
  - paese/francia
  - comp/coupe-animath
  - anno/2021
  - livello/Coupe-Animath-Printemps
  - stagione/printemps
  - topic/combinatoria
  - topic/geometria_solida
  - gara/individuale
---

*8 ants on the edges of a unit cube: two at geodesic distance at most 1 (lycee)*

> $8$ ants are placed on the edges of a cube whose edges have length $1$. For every positive real $d$, two ants are said to be at distance $d$ if the first ant must travel a distance of at least $d$ to reach the second ant, moving only along the edges of the cube. Prove that there exist two ants at distance less than or equal to $1$.

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_geometria_solida|Geometria solida]]
**Metodo:** [[method_cassetti|Principio dei cassetti]], [[method_estremalita|Estremalità]]
**Abilita:** [[skill_modellizzazione|Modellizzazione]], [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_stima|Stima]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Geometria]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1HT9T8KwtrtEAIUZX2Qh9AX2VGVFmNAVg/view)

---
tipo: quesito
quesito_id: quesito_src_obm_2007_n1_f1__Q16
parent: src_obm_2007_n1_f1
competition: Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1
family: obm
year: '2007'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '16'
summary: Grid of 1 cm2 squares; segments join midpoints to spell OBM; find shaded area
qa_score: '2'
topics:
  - topic_geometria_piana
methods:
  - method_conteggio
skills:
  - skill_ragionamento_geometrico
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2007
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_piana
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1 — Quesito 16

*Grid of 1 cm2 squares; segments join midpoints to spell OBM; find shaded area*

> In the grid, each little square has area $1\text{ cm}^2$. Segments join midpoints of the sides of the squares at the center of each small square, forming the word OBM. What is the area occupied by the OBM logo, in $\text{cm}^2$?
> 
> (A) 28 \quad (B) 32 \quad (C) 33 \quad (D) 34 \quad (E) 35

![[src_obm_2007_n1_f1__Q16.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_conteggio|Conteggio]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Geometria]]
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

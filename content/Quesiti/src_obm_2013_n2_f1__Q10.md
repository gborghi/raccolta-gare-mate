---
tipo: quesito
quesito_id: quesito_src_obm_2013_n2_f1__Q10
parent: src_obm_2013_n2_f1
competition: Olimpíada Brasileira de Matemática 2013 — Nível 2 — Fase 1
family: obm
year: '2013'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '10'
summary: 'Triangle of consecutive odd numbers: which row contains 2013?'
qa_score: '3'
answer: D
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_ricorsione
  - method_telescoping
skills:
  - skill_riconoscimento_pattern
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2013
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2013 — Nível 2 — Fase 1 — Quesito 10

*Triangle of consecutive odd numbers: which row contains 2013?*

> The arithmetic triangle of Fibonacci is formed by placing all positive odd integers, starting from 1, row by row, with each row containing one more element than the previous. The first four rows are:
> 
> $$\text{Row 1: } 1$$
> $$\text{Row 2: } 3 \quad 5$$
> $$\text{Row 3: } 7 \quad 9 \quad 11$$
> $$\text{Row 4: } 13 \quad 15 \quad 17 \quad 19$$
> 
> In which row does the number $2013$ appear?
> 
> (A) 45 \quad (B) 46 \quad (C) 62 \quad (D) 63 \quad (E) 64

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_ricorsione|Ricorsione]], [[method_telescoping|Telescoping]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Risposta:** D
**Fonte:** apri PDF

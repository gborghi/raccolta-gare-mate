---
title: Olimpíada Brasileira de Matemática 2011 — Nível 1 — Fase 1 — Quesito 8
tipo: quesito
quesito_id: quesito_src_obm_2011_n1_f1__Q08
parent: src_obm_2011_n1_f1
competition: Olimpíada Brasileira de Matemática 2011 — Nível 1 — Fase 1
family: obm
year: '2011'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '8'
summary: Area of rectangle divided into 10 squares with smallest integer sides
qa_score: '2'
answer: C
topics:
  - topic_geometria_piana
  - topic_algebra
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2011
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_piana
  - topic/algebra
  - gara/individuale
  - qa/da-verificare
---

*Area of rectangle divided into 10 squares with smallest integer sides*

> The rectangle in the figure below is divided into 10 squares. The measures of the sides of all the squares are positive integers and are the smallest possible values. The area of this rectangle is:
> 
> (A) 180 \quad (B) 240 \quad (C) 300 \quad (D) 360 \quad (E) 450

![[src_obm_2011_n1_f1__Q08.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]], [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]], [[Algebra e Analisi]]
**Risposta:** C
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

---
title: China Mathematical Competition 2004 — Quesito 3
tipo: quesito
quesito_id: quesito_src_cn_cmc_2004__Q03
parent: src_cn_cmc_2004
competition: China Mathematical Competition 2004
family: cmc
year: '2004'
level: China Mathematical Competition
country: Cina
modalita: individuale
quesito: '3'
summary: Solution set of inequality with square root and log
qa_score: '4'
answer: C
topics:
  - topic_algebra
  - topic_insiemi_funzioni
methods:
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/cina
  - comp/cmc
  - anno/2004
  - livello/China-Mathematical-Competition
  - topic/algebra
  - topic/insiemi_funzioni
  - gara/individuale
---

*Solution set of inequality with square root and log*

> The solution set of the inequality $\sqrt{\log_2 x - 1} + \dfrac{1}{2}\log_{\frac{1}{2}} x^3 + 2 > 0$ is ( ).
> 
> (A) $[2,\, 3)$ \quad (B) $(2,\, 3]$ \quad (C) $[2,\, 4)$ \quad (D) $(2,\, 4]$

**Topic:** [[topic_algebra|Algebra]], [[topic_insiemi_funzioni|Insiemi e funzioni]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Algebra e Analisi]]
**Risposta:** C
**Fonte:** apri libro PDF

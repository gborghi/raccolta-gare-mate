---
tipo: quesito
quesito_id: quesito_src_imo_1995__Q03
parent: src_imo_1995
competition: 'IMO 1995 (36th IMO, Toronto)'
family: imo
year: '1995'
level: IMO
country: Italia
quesito: '3'
summary: Determine n with point areas equal to ri+rj+rk
topics:
  - topic_geometria_piana
  - topic_combinatoria
methods:
  - method_coordinate
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/1995
  - livello/IMO
  - topic/geometria_piana
  - topic/combinatoria
---

# IMO 1995 (36th IMO, Toronto) — Quesito 3

*Determine n with point areas equal to ri+rj+rk*

> Determine all integers n > 3 for which there exist n points A1, . . . , An
> in the plane, no three collinear, and real numbers r1, . . . , rn such that
> for 1 ≤i < j < k ≤n, the area of △AiAjAk is ri + rj + rk.
> 36th International Mathematical Olympiad
> Second Day - Toronto - July 20, 1995
> Time Limit: 41
> 2 hours

**Topic:** [[topic_geometria_piana|Geometria piana]], [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_coordinate|Metodo delle coordinate]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Geometria]]
**Fonte:** apri PDF p.1

---
tipo: quesito
quesito_id: quesito_src_obm_2004_nu_f1__Q01
parent: src_obm_2004_nu_f1
competition: Olimpíada Brasileira de Matemática 2004 — Nível Universitário — Fase 1
family: obm
year: '2004'
level: OBM Nível Universitário
country: Brasile
modalita: individuale
nivel: u
fase: '1'
quesito: '1'
summary: Compute A^2004 for a given complex matrix
qa_score: '4'
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
  - method_induzione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-Universitário
  - nivel/u
  - fase/1
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2004 — Nível Universitário — Fase 1 — Quesito 1

*Compute A^2004 for a given complex matrix*

> Consider the complex matrix $A = \begin{pmatrix} 1 & 0 & i \\ 0 & 0 & 0 \\ i & 0 & 1 \end{pmatrix}$. Compute $A^{2004}$.

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]], [[method_induzione|Induzione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

---
tipo: quesito
quesito_id: quesito_src_jmo31yq_yosen__Q01
parent: src_jmo31yq_yosen
competition: Japan Mathematical Olympiad 2021 — Yosen (Preliminary)
family: jmo
year: '2021'
level: JMO Yosen
country: Giappone
modalita: individuale
quesito: '1'
summary: 'Maximize product mn with m+n=90 and m,n coprime'
qa_score: '4'
answer: '2021'
topics:
  - topic_aritmetica
methods:
  - method_estremalita
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/giappone
  - comp/jmo
  - anno/2021
  - livello/JMO-Yosen
  - topic/aritmetica
  - gara/individuale
---

# Japan Mathematical Olympiad 2021 — Yosen (Preliminary) — Quesito 1

*Maximize product mn with m+n=90 and m,n coprime*

> Let $m$ and $n$ be mutually coprime positive integers satisfying $m+n=90$. Find the maximum possible value of the product $mn$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_estremalita|Estremalità]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** 2021
**Fonte:** apri PDF

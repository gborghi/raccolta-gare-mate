---
tipo: quesito
quesito_id: quesito_src_kangourou_2019_student_marzo__Q22
parent: src_kangourou_2019_student_marzo
competition: Kangourou 2019 Gara marzo Student (triennio)
family: kangourou
year: '2019'
level: kangourou
country: Italia
quesito: '22'
summary: Per quali a l equazione 2-|x|=ax ha due soluzioni
answer: B
topics:
  - topic_algebra
methods:
  - method_casework
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/italia
  - comp/kangourou
  - anno/2019
  - livello/kangourou
  - topic/algebra
---

# Kangourou 2019 Gara marzo Student (triennio) — Quesito 22

*Per quali a l equazione 2-|x|=ax ha due soluzioni*

> L’equazione 2 – | x | = ax  ha esattamente due soluzioni distinte se e solo se
> A) a ≤ 1.	
> 	
> B) –1 < a < 1.	
> 	
> C) a ≥ 1.	
> D) a = 0.	
> 	
> E) a = –1  oppure  a = 1.
> a + b
> -------- 
>    c
> 
> Kangourou 2019
> pagina 30
> STUDENT

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Analisi per casi]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** B
**Fonte:** apri PDF p.4

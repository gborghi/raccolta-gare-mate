---
tipo: quesito
quesito_id: quesito_src_obm_2006_n2_f1__Q05
parent: src_obm_2006_n2_f1
competition: Olimpíada Brasileira de Matemática 2006 — Nível 2 — Fase 1
family: obm
year: '2006'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '5'
summary: 'Isosceles triangle with angle BAD=30 degrees, find angle x'
qa_score: '3'
answer: C
topics:
  - topic_geometria_piana
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2006
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/geometria_piana
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2006 — Nível 2 — Fase 1 — Quesito 5

*Isosceles triangle with angle BAD=30 degrees, find angle x*

> In the figure, $AB = AC$, $AE = AD$, and the angle $BAD = 30^\circ$. Then the angle $x$ measures:
> 
> **A)** $10^\circ$ \quad **B)** $20^\circ$ \quad **C)** $15^\circ$ \quad **D)** $30^\circ$ \quad **E)** $5^\circ$

![[src_obm_2006_n2_f1__Q05.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Geometria]]
**Risposta:** C
**Fonte:** apri PDF

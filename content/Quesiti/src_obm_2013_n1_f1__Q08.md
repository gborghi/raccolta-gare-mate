---
title: Olimpíada Brasileira de Matemática 2013 — Nível 1 — Fase 1 — Quesito 8
tipo: quesito
quesito_id: quesito_src_obm_2013_n1_f1__Q08
parent: src_obm_2013_n1_f1
competition: Olimpíada Brasileira de Matemática 2013 — Nível 1 — Fase 1
family: obm
year: '2013'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '8'
summary: Triangle pattern of black/white circles; black circles when 30 are white
qa_score: '4'
topics:
  - topic_combinatoria
  - topic_aritmetica
methods:
  - method_conteggio
  - method_casework
skills:
  - skill_riconoscimento_pattern
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2013
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/combinatoria
  - topic/aritmetica
  - gara/individuale
---

*Triangle pattern of black/white circles; black circles when 30 are white*

> White and black circles are used to construct triangles as in the figure. We begin with a black circle in the first row. From there on, the even rows are formed only by white circles, and the odd rows by circles of alternating colors, beginning with a black circle at the tip. If a triangle like this has exactly $30$ white circles, how many black circles does it have?
> 
> (A) $10$ \quad (B) $15$ \quad (C) $18$ \quad (D) $20$ \quad (E) $30$

![[src_obm_2013_n1_f1__Q08.png]]

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_conteggio|Conteggio]], [[method_casework|Casework]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

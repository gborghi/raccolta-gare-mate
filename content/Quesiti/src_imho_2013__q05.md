...
quesito: '5'
summary: Functional equation on positive rationals with f(x)=x
qa_score: '5'
topics:
  - topic_funzionali
  - topic_algebra
...
> \begin{itemize}
> \item[(i)] for all $x, y \in \mathbb{Q}_{>0}$, we have $f(x) \cdot f(y) \ge f(xy)$;
> \item[(ii)] for all $x, y \in \mathbb{Q}_{>0}$, we have $f(x + y) \ge f(x) + f(y)$;

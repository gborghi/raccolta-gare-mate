---
title: China Girls' Mathematical Olympiad 2012 — Quesito 1
tipo: quesito
quesito_id: quesito_src_cn_cgmo_2012__Q01
parent: src_cn_cgmo_2012
competition: China Girls' Mathematical Olympiad 2012
family: cgmo
year: '2012'
level: China Girls' Mathematical Olympiad
country: Cina
modalita: individuale
quesito: '1'
summary: Prove a telescoping inequality for non-negative reals
qa_score: '3'
topics:
  - topic_disuguaglianze
  - topic_algebra
methods:
  - method_induzione
  - method_telescoping
skills:
  - skill_manipolazione_algebrica
  - skill_ragionamento_geometrico
tags:
  - kg/quesito
  - paese/cina
  - comp/cgmo
  - anno/2012
  - livello/China-Girls'-Mathematical-Olympiad
  - topic/disuguaglianze
  - topic/algebra
  - gara/individuale
---

*Prove a telescoping inequality for non-negative reals*

> Let $a_1$, $a_2$, $\ldots$, $a_n$ be $n$ non-negative real numbers. Prove that
> $$\frac{1}{1+a_1} + \frac{a_1}{(1+a_1)(1+a_2)} + \cdots + \frac{a_1 a_2 \cdots a_{n-1}}{(1+a_1)(1+a_2)\cdots(1+a_n)} \le 1.$$
> (posed by Ai Yinghua)

**Topic:** [[topic_disuguaglianze|Disuguaglianze]], [[topic_algebra|Algebra]]
**Metodo:** [[method_induzione|Induzione]], [[method_telescoping|Telescoping]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_ragionamento_geometrico|Ragionamento geometrico]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri libro PDF](https://drive.google.com/file/d/1cjmD7lTT4vb3nB3ylcVCHeHucUE2rNI0/view)

---
tipo: quesito
quesito_id: quesito_src_obm_2014_n3_f1__Q15
parent: src_obm_2014_n3_f1
competition: Olimpíada Brasileira de Matemática 2014 — Nível 3 — Fase 1
family: obm
year: '2014'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '15'
summary: Sum of the roots of 1/(1+x)+2/(2+x)+3/(3+x)=1
qa_score: '4'
topics:
  - topic_algebra
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2014
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2014 — Nível 3 — Fase 1 — Quesito 15

*Sum of the roots of 1/(1+x)+2/(2+x)+3/(3+x)=1*

> The sum of the roots of the equation $\frac{1}{1+x} + \frac{2}{2+x} + \frac{3}{3+x} = 1$ is:
> (A) $0$ (B) $6$ (C) $14$ (D) $11$ (E) $9$

**Topic:** [[topic_algebra|Algebra]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

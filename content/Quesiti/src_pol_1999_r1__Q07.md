---
title: Olimpiade Matematica Polacca 1999 — Round 1 — Quesito 7
tipo: quesito
quesito_id: quesito_src_pol_1999_r1__Q07
parent: src_pol_1999_r1
competition: Olimpiade Matematica Polacca 1999 — Round 1
family: pol
year: '1999'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '7'
summary: >-
  Find all degree-n polynomials with exactly n real roots not exceeding -1
  satisfying a_0^2+a_1 a_n=a_n^2+a_0 a_{n-1}.
qa_score: '5'
topics:
  - topic_algebra
methods:
  - method_disuguaglianze
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_astrazione
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/1999
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/algebra
  - gara/individuale
---

*Find all degree-n polynomials with exactly n real roots not exceeding -1 satisfying a_0^2+a_1 a_n=a_n^2+a_0 a_{n-1}.*

> Let $n \ge 2$ be an integer. Find all polynomials $P(x) = a_0 + a_1 x + \cdots + a_n x^n$ having exactly $n$ real roots not exceeding $-1$ and satisfying $$a_0^2 + a_1 a_n = a_n^2 + a_0 a_{n-1}.$$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_astrazione|Astrazione]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

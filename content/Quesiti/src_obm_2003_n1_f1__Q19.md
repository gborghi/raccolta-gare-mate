---
title: Olimpíada Brasileira de Matemática 2003 — Nível 1 — Fase 1 — Quesito 19
tipo: quesito
quesito_id: quesito_src_obm_2003_n1_f1__Q19
parent: src_obm_2003_n1_f1
competition: Olimpíada Brasileira de Matemática 2003 — Nível 1 — Fase 1
family: obm
year: '2003'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '19'
summary: How many cells did Camila mark in the board game with Lara
qa_score: '3'
answer: B
topics:
  - topic_logica
  - topic_combinatoria
methods:
  - method_casework
skills:
  - skill_lettura_attenta
  - skill_ragionamento_geometrico
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2003
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/logica
  - topic/combinatoria
  - gara/individuale
---

*How many cells did Camila mark in the board game with Lara*

> Camila and Lara are playing a game on a $4 \times 4$ board. Camila marks some cells of the board and tells Lara the number of marked cells in the neighbourhood (sharing a side or vertex, i.e., an edge or corner) of each cell. In this game, two distinct cells are considered neighbours if they share a side or a corner. Lara must find out which cells were marked by Camila. After marking some cells, Camila passes the following board to Lara:
> 
> $$\begin{array}{|c|c|c|c|}
> \hline
> 1 & 2 & 1 & 1 \\
> \hline
> 0 & 2 & 1 & 2 \\
> \hline
> 2 & 3 & 3 & 1 \\
> \hline
> 1 & 0 & 2 & 1 \\
> \hline
> \end{array}$$
> 
> The number of cells marked by Camila is:
> 
> (A) 3 \quad (B) 4 \quad (C) 5 \quad (D) 6 \quad (E) 7

**Topic:** [[topic_logica|Logica]], [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** B
**Fonte:** apri PDF

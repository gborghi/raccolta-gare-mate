---
tipo: quesito
quesito_id: quesito_src_obm_1998_n2_f1__Q12
parent: src_obm_1998_n2_f1
competition: Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1
family: obm
year: '1998'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '12'
summary: Minimum string length to tie 15 packages
qa_score: '3'
answer: B
topics:
  - topic_geometria_solida
  - topic_aritmetica
skills:
  - skill_modellizzazione
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1998
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/geometria_solida
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 1998 — Nível 2 — Fase 1 — Quesito 12

*Minimum string length to tie 15 packages*

> What is the minimum number of metres of string needed to tie 15 packages, given that each package measures $10\,\mathrm{cm} \times 20\,\mathrm{cm} \times 40\,\mathrm{cm}$ and 20 cm of string is reserved for the bow?
> 
> A) 39
> B) 36
> C) 48
> D) 56
> E) 42

![[src_obm_1998_n2_f1__Q12.png]]

**Topic:** [[topic_geometria_solida|Geometria solida]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Abilita:** [[skill_modellizzazione|Modellizzazione]], [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Geometria]], [[Aritmetica e Teoria dei Numeri]]
**Risposta:** B
**Fonte:** apri PDF

---
title: Olimpíada Brasileira de Matemática 2001 — Nível 1 — Fase 1 — Quesito 2
tipo: quesito
quesito_id: quesito_src_obm_2001_n1_f1__Q02
parent: src_obm_2001_n1_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 1 — Fase 1
family: obm
year: '2001'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '2'
summary: >-
  Count non-crossing quadrilaterals formed by joining one point from each of
  four circles containing 1,2,3,4 interior points
qa_score: '4'
answer: D
topics:
  - topic_combinatoria
methods:
  - method_conteggio
skills:
  - skill_conteggio_sistematico
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/combinatoria
  - gara/individuale
---

*Count non-crossing quadrilaterals formed by joining one point from each of four circles containing 1,2,3,4 interior points*

> In the figure below we have $4$ circumferences and some highlighted points in the interior of these circumferences. The circumferences contain in their interior, respectively, $1$, $2$, $3$ and $4$ of these highlighted points, and by joining $4$ of these points in such a way that they do not cross, distinct quadrilaterals are formed. How many distinct quadrilaterals can we draw under these conditions?
> 
> (A) $4$ \quad (B) $14$ \quad (C) $60$ \quad (D) $120$ \quad (E) $24$

![[src_obm_2001_n1_f1__Q02.png]]

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_conteggio|Conteggio]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Risposta:** D
**Fonte:** apri PDF

---
title: Olimpíada Brasileira de Matemática 2001 — Nível 3 — Fase 1 — Quesito 18
tipo: quesito
quesito_id: quesito_src_obm_2001_n3_f1__Q18
parent: src_obm_2001_n3_f1
competition: Olimpíada Brasileira de Matemática 2001 — Nível 3 — Fase 1
family: obm
year: '2001'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '18'
summary: f(x)=x^2-3x+4; count real solutions of f applied 2001 times equals 2
qa_score: '4'
answer: C
topics:
  - topic_funzionali
  - topic_algebra
methods:
  - method_backward
  - method_ricorsione
skills:
  - skill_riconoscimento_pattern
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2001
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/funzionali
  - topic/algebra
  - gara/individuale
---

*f(x)=x^2-3x+4; count real solutions of f applied 2001 times equals 2*

> Let $f(x) = x^2 - 3x + 4$. How many real solutions does the equation $f(f(f(\ldots f(x)\ldots))) = 2$ have (where $f$ is applied 2001 times)?
> 
> **A)** 0 $\quad$ **B)** 1 $\quad$ **C)** 2 $\quad$ **D)** 2001 $\quad$ **E)** $2^{2001}$

**Topic:** [[topic_funzionali|Equazioni funzionali]], [[topic_algebra|Algebra]]
**Metodo:** [[method_backward|Backward]], [[method_ricorsione|Ricorsione]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** C
**Fonte:** apri PDF

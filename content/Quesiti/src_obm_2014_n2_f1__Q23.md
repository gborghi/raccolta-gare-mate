---
tipo: quesito
quesito_id: quesito_src_obm_2014_n2_f1__Q23
parent: src_obm_2014_n2_f1
competition: Olimpíada Brasileira de Matemática 2014 — Nível 2 — Fase 1
family: obm
year: '2014'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '23'
summary: 'Given sqrt(x-y)=a and sqrt(x)+sqrt(y)=b, find sqrt(xy)'
qa_score: '3'
answer: A
topics:
  - topic_algebra
methods:
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2014
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/algebra
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2014 — Nível 2 — Fase 1 — Quesito 23

*Given sqrt(x-y)=a and sqrt(x)+sqrt(y)=b, find sqrt(xy)*

> If $a$, $b$, $x$, $y$ are positive reals such that $\sqrt{x - y} = a$ and $\sqrt{x} + \sqrt{y} = b$, determine the value of $\sqrt{xy}$.
> 
> (A) $\dfrac{b^2 - a^2}{4}$ $\quad$ (B) $\dfrac{a^2}{b}$ $\quad$ (C) $\dfrac{b^2 + a^2}{4b}$ $\quad$ (D) $\dfrac{1}{b}$ $\quad$ (E) $a^2$

**Topic:** [[topic_algebra|Algebra]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]]
**Risposta:** A
**Fonte:** apri PDF

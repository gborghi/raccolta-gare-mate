---
tipo: quesito
quesito_id: quesito_src_obm_1998_n3_f1__Q13
parent: src_obm_1998_n3_f1
competition: Olimpíada Brasileira de Matemática 1998 — Nível 3 — Fase 1
family: obm
year: '1998'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '13'
summary: >-
  In a set of points in space where any two distinct points are at distance 1,
  find the maximum number of points.
qa_score: '3'
answer: C
topics:
  - topic_geometria_solida
methods:
  - method_estremalita
skills:
  - skill_ragionamento_geometrico
  - skill_astrazione
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/1998
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/geometria_solida
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 1998 — Nível 3 — Fase 1 — Quesito 13

*In a set of points in space where any two distinct points are at distance 1, find the maximum number of points.*

> In a set of points in space, the distance between any two different points is equal to $1$. The maximum number of points that this set can have is:
> 
> A) 2
> 
> B) 3
> 
> C) 4
> 
> D) 6
> 
> E) 8

**Topic:** [[topic_geometria_solida|Geometria solida]]
**Metodo:** [[method_estremalita|Estremalità]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_astrazione|Astrazione]]
**Area:** [[Geometria]]
**Risposta:** C
**Fonte:** apri PDF

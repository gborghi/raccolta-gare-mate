---
tipo: quesito
quesito_id: quesito_src_obm_2011_n2_f1__Q09
parent: src_obm_2011_n2_f1
competition: Olimpíada Brasileira de Matemática 2011 — Nível 2 — Fase 1
family: obm
year: '2011'
level: OBM Nível 2
country: Brasile
modalita: individuale
nivel: '2'
fase: '1'
quesito: '9'
summary: Count positive numbers below 30 with exactly two divisors (primes)
qa_score: '3'
topics:
  - topic_aritmetica
methods:
  - method_conteggio
skills:
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2011
  - livello/OBM-Nível-2
  - nivel/2
  - fase/1
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2011 — Nível 2 — Fase 1 — Quesito 9

*Count positive numbers below 30 with exactly two divisors (primes)*

> How many positive numbers smaller than $30$ have exactly two positive divisors?
> 
> (A) $6$ \quad (B) $7$ \quad (C) $8$ \quad (D) $9$ \quad (E) $10$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_conteggio|Conteggio]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

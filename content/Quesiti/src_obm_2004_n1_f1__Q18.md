---
tipo: quesito
quesito_id: quesito_src_obm_2004_n1_f1__Q18
parent: src_obm_2004_n1_f1
competition: Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1
family: obm
year: '2004'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '18'
summary: Which 3D shape is formed by folding and joining paper pieces?
qa_score: '3'
answer: B
topics:
  - topic_geometria_solida
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_astrazione
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_solida
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1 — Quesito 18

*Which 3D shape is formed by folding and joining paper pieces?*

> A cube can be constructed from two pieces of cardboard presented as alternatives below, by folding only along the dashed lines and joining along the solid lines. These two pieces are:
> 
> (A) Pieces A \quad (B) Pieces B \quad (C) Pieces C \quad (D) Pieces D \quad (E) Pieces E

![[src_obm_2004_n1_f1__Q18.png]]

**Topic:** [[topic_geometria_solida|Geometria solida]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_astrazione|Astrazione]]
**Area:** [[Geometria]]
**Risposta:** B
**Fonte:** apri PDF

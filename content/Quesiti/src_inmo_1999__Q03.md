...
quesito: '3'
summary: >-
  Nonconstant polynomials p(x) and q(x) with integer coefficients satisfying
  p(x)q(x) = x^4 + 2x + 1
qa_score: '5'
topics:
  - topic_algebra
...
> Prove that there are no nonconstant polynomials $p(x)$ and $q(x)$ with integer coefficients such that $p(x)q(x) = x^4 + 2x + 1$.

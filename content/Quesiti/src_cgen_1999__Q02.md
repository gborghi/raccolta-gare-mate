---
tipo: quesito
quesito_id: quesito_src_cgen_1999__Q02
parent: src_cgen_1999
competition: Concours Général des Lycées 1999 — Matematica
family: concours_generale
year: '1999'
level: Concours Général
country: Francia
modalita: individuale
quesito: '2'
summary: Solve in N the equation involving sum of powers
qa_score: '5'
topics:
  - topic_aritmetica
  - topic_algebra
methods:
  - method_casework
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
  - skill_lettura_attenta
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/francia
  - comp/concours-generale
  - anno/1999
  - livello/Concours-Général
  - topic/aritmetica
  - topic/algebra
  - gara/individuale
---

# Concours Général des Lycées 1999 — Matematica — Quesito 2

*Solve in N the equation involving sum of powers*

> Solve in $\mathbb{N}$ the equation in $n$:
> $$( n+3)^{n} = \sum_{k=1}^{n+2} k^{n}$$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Casework]], [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_lettura_attenta|Lettura attenta]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Algebra e Analisi]]
**Fonte:** apri PDF

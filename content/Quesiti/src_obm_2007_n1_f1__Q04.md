---
tipo: quesito
quesito_id: quesito_src_obm_2007_n1_f1__Q04
parent: src_obm_2007_n1_f1
competition: Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1
family: obm
year: '2007'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '4'
summary: Two overlapping squares of area 100 with total perimeter 50; find common area
qa_score: '2'
topics:
  - topic_geometria_piana
skills:
  - skill_ragionamento_geometrico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2007
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_piana
  - gara/individuale
  - qa/da-verificare
---

# Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1 — Quesito 4

*Two overlapping squares of area 100 with total perimeter 50; find common area*

> The figure beside is formed by two squares, each of area $100\text{ cm}^2$, partially overlapping, in such a way that the perimeter of the figure (the thick line) is equal to $50$ cm. What is the area of the region common to the two squares, in $\text{cm}^2$?
> 
> (A) 20 \quad (B) 25 \quad (C) 30 \quad (D) 40 \quad (E) 50

![[src_obm_2007_n1_f1__Q04.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Geometria]]
**Fonte:** apri PDF

> [!warning] Estratto/tradotto da verificare con la fonte.

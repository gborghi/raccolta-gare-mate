...
quesito: '3'
summary: >-
  Given n points on the coordinate axes with integer coordinates bounded by n,
  minimize the cyclic sum of squared distances between consecutive points.
qa_score: '2'
topics:
  - topic_combinatoria
...
> $$\{P_0, P_1, \ldots, P_n\} = \{(x, y) \mid x \text{ and } y \text{ are integers with } xy = 0, |x| \le n, |y| \le n\}.$$
> Determine the minimum value of
> $$(P_0 P_1)^2 + (P_1 P_2)^2 + \cdots + (P_{n-1} P_n)^2 + (P_n P_0)^2.$$
> (Posed by Wang Xinmiao)

---
tipo: quesito
quesito_id: quesito_src_jjmo14yq_yosen__Q06
parent: src_jjmo14yq_yosen
competition: Japan Junior Mathematical Olympiad 2016 — Yosen (Preliminary)
family: jjmo
year: '2016'
level: JJMO Yosen
country: Giappone
modalita: individuale
quesito: '6'
summary: Maximize m for positive integers with m(m+57)=n(n+57)
qa_score: '2'
answer: '783'
topics:
  - topic_algebra
  - topic_aritmetica
methods:
  - method_fattorizzazione
skills:
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/giappone
  - comp/jjmo
  - anno/2016
  - livello/JJMO-Yosen
  - topic/algebra
  - topic/aritmetica
  - gara/individuale
  - qa/da-verificare
---

# Japan Junior Mathematical Olympiad 2016 — Yosen (Preliminary) — Quesito 6

*Maximize m for positive integers with m(m+57)=n(n+57)*

> Positive integers $m,n$ satisfy
> $$m(m+57) = n(n+57).$$
> Among such pairs, find the maximum possible value of $m$.

**Topic:** [[topic_algebra|Algebra]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_fattorizzazione|Fattorizzazione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Algebra e Analisi]], [[Aritmetica e Teoria dei Numeri]]
**Risposta:** 783
**Fonte:** apri PDF

> [!warning] Estratto da scansione giapponese a bassa risoluzione — enunciato tradotto da verificare con la fonte.

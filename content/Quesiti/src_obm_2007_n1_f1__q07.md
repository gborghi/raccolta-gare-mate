---
title: Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1 — Quesito 7
tipo: quesito
quesito_id: quesito_src_obm_2007_n1_f1__Q07
parent: src_obm_2007_n1_f1
competition: Olimpíada Brasileira de Matemática 2007 — Nível 1 — Fase 1
family: obm
year: '2007'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '7'
summary: Fraction a/b between 0 and 1/7 from number line position; find possible a+b
qa_score: '2'
topics:
  - topic_aritmetica
methods:
  - method_casework
skills:
  - skill_lettura_attenta
  - skill_stima
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2007
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/aritmetica
  - gara/individuale
  - qa/da-verificare
---

*Fraction a/b between 0 and 1/7 from number line position; find possible a+b*

> The fraction $\dfrac{a}{b}$, where $a$ and $b$ are positive integers, represents a number between $0$ and $\dfrac{1}{7}$, in the position shown in the diagram beside. What is a possible value for the sum $a + b$?
> 
> (A) 1 \quad (B) 2 \quad (C) 3 \quad (D) 4 \quad (E) 5

![[src_obm_2007_n1_f1__Q07.png]]

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_lettura_attenta|Lettura attenta]], [[skill_stima|Stima]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** [apri PDF](https://drive.google.com/file/d/1WxbsaFDc4UiPT0pYHWe1rUvNJyqwhBV8/view)

> [!warning] Estratto/tradotto da verificare con la fonte.

---
title: Olimpiade Matematica Polacca 2002 — Round 1 — Quesito 12
tipo: quesito
quesito_id: quesito_src_pol_2002_r1__Q12
parent: src_pol_2002_r1
competition: Olimpiade Matematica Polacca 2002 — Round 1
family: pol
year: '2002'
level: Olimpiade Polacca Round 1
round: '1'
country: Polonia
modalita: individuale
quesito: '12'
summary: >-
  A non-decreasing sequence of positive integers has exactly k values equal to
  k; find all primes among its partial sums.
qa_score: '4'
topics:
  - topic_aritmetica
  - topic_combinatoria
methods:
  - method_conteggio
skills:
  - skill_riconoscimento_pattern
  - skill_conteggio_sistematico
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/polonia
  - comp/pol
  - anno/2002
  - livello/olimpiade-polacca-round-1
  - round/1
  - topic/aritmetica
  - topic/combinatoria
  - gara/individuale
---

*A non-decreasing sequence of positive integers has exactly k values equal to k; find all primes among its partial sums.*

> In a non-decreasing sequence $a_1, a_2, a_3, \ldots$ all values are positive integers and exactly $k$ values are equal to $k$. Find all primes of the form
> $$a_1 + a_2 + \cdots + a_n.$$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]], [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_conteggio|Conteggio]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Aritmetica e Teoria dei Numeri]], [[Combinatoria, Logica e Probabilita]]
**Fonte:** apri PDF

---
tipo: quesito
quesito_id: quesito_src_bmo_2004-05_round2__Q03
parent: src_bmo_2004-05_round2
competition: British Mathematical Olympiad 2004-05 — Round 2
family: bmo
year: '2005'
level: BMO Round 2
country: Regno Unito
modalita: individuale
quesito: '3'
summary: 'Prove inequality for positive reals a, b with a+b=1'
qa_score: '5'
topics:
  - topic_disuguaglianze
  - topic_algebra
methods:
  - method_disuguaglianze
skills:
  - skill_manipolazione_algebrica
  - skill_stima
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/bmo
  - anno/2005
  - livello/BMO-Round-2
  - topic/disuguaglianze
  - topic/algebra
  - gara/individuale
---

# British Mathematical Olympiad 2004-05 — Round 2 — Quesito 3

*Prove inequality for positive reals a, b with a+b=1*

> Let $a, b$ be positive real numbers with $a + b = 1$. Prove that
> $$a^a b^b + a^b b^a \le 1.$$

**Topic:** [[topic_disuguaglianze|Disuguaglianze]], [[topic_algebra|Algebra]]
**Metodo:** [[method_disuguaglianze|Disuguaglianze]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_stima|Stima]]
**Area:** [[Algebra e Analisi]]
**Fonte:** apri PDF

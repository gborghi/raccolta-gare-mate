---
title: IMO 1973 (Fifteenth International Olympiad) — Quesito 5
tipo: quesito
quesito_id: quesito_src_imo_1973_all__Q05
parent: src_imo_1973_all
competition: IMO 1973 (Fifteenth International Olympiad)
family: imo
year: '1973'
level: IMO
country: Italia
modalita: individuale
quesito: '5'
summary: Group of affine maps has common fixed point
topics:
  - topic_insiemi_funzioni
methods:
  - method_invarianti
skills:
  - skill_astrazione
tags:
  - kg/quesito
  - paese/italia
  - comp/imo
  - anno/1973
  - livello/IMO
  - topic/insiemi_funzioni
  - gara/individuale
---

*Group of affine maps has common fixed point*

> $G$ è un insieme di funzioni non costanti della variabile reale $x$ della forma
> $$f(x) = ax + b, \quad a, b \in \mathbb{R},$$
> e $G$ gode delle seguenti proprietà:
> 
> (a) Se $f$ e $g$ appartengono a $G$, allora $g \circ f$ appartiene a $G$; qui $(g \circ f)(x) = g[f(x)]$.
> 
> (b) Se $f$ appartiene a $G$, allora la sua inversa $f^{-1}$ appartiene a $G$; qui l'inversa di $f(x) = ax + b$ è $f^{-1}(x) = \dfrac{x - b}{a}$.
> 
> (c) Per ogni $f \in G$ esiste un numero reale $x_f$ tale che $f(x_f) = x_f$.
> 
> Dimostrare che esiste un numero reale $k$ tale che $f(k) = k$ per ogni $f \in G$.

**Topic:** [[topic_insiemi_funzioni|Insiemi e funzioni]]
**Metodo:** [[method_invarianti|Invarianti / monovarianti]]
**Abilita:** [[skill_astrazione|Astrazione / generalizzazione]]
**Area:** [[Algebra e Analisi]]
**Fonte:** [apri PDF p.1](https://drive.google.com/file/d/1_yg5ytepW4kPtXzf2MZzjG__z3aJ2dcR/view)

---
title: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1 — Quesito 5
tipo: quesito
quesito_id: quesito_src_obm_2005_n1_f1__Q05
parent: src_obm_2005_n1_f1
competition: Olimpíada Brasileira de Matemática 2005 — Nível 1 — Fase 1
family: obm
year: '2005'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '5'
summary: Sixth term of a sequence where each term equals sum of two previous
qa_score: '3'
answer: B
topics:
  - topic_aritmetica
methods:
  - method_ricorsione
skills:
  - skill_manipolazione_algebrica
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2005
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/aritmetica
  - gara/individuale
---

*Sixth term of a sequence where each term equals sum of two previous*

> In a sequence, each term from the third onward is the sum of the two preceding terms. The second term equals $1$ and the fifth term equals $2005$. What is the sixth term?
> 
> A) $3{,}002$ \quad B) $3{,}008$ \quad C) $3{,}010$ \quad D) $4{,}002$ \quad E) $5{,}004$

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_ricorsione|Ricorsione]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Risposta:** B
**Fonte:** apri PDF

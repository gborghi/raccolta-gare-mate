---
title: Olimpíada Brasileira de Matemática 2010 — Nível 1 — Fase 1 — Quesito 20
tipo: quesito
quesito_id: quesito_src_obm_2010_n1_f1__Q20
parent: src_obm_2010_n1_f1
competition: Olimpíada Brasileira de Matemática 2010 — Nível 1 — Fase 1
family: obm
year: '2010'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '20'
summary: Find the missing face number on an icosahedron net
qa_score: '3'
answer: D
topics:
  - topic_combinatoria
  - topic_geometria_solida
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
  - skill_conteggio_sistematico
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2010
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/combinatoria
  - topic/geometria_solida
  - gara/individuale
---

*Find the missing face number on an icosahedron net*

> The figure below is a net (plan) of an icosahedron (a solid all of whose faces are equilateral triangles). The faces are numbered $1$ to $5$ in such a way that around each vertex of the solid the numbers $1$ to $5$ all appear. What number is on the face marked with ``?''?
> 
> (A) $1$ \quad (B) $2$ \quad (C) $3$ \quad (D) $4$ \quad (E) $5$

![[src_obm_2010_n1_f1__Q20.png]]

**Topic:** [[topic_combinatoria|Combinatoria]], [[topic_geometria_solida|Geometria solida]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]], [[skill_conteggio_sistematico|Conteggio sistematico]]
**Area:** [[Combinatoria, Logica e Probabilita]], [[Geometria]]
**Risposta:** D
**Fonte:** apri PDF

---
title: British Mathematical Olympiad 1996-97 — Round 1 — Quesito 2
tipo: quesito
quesito_id: quesito_src_bmo_1996-97_round1__Q02
parent: src_bmo_1996-97_round1
competition: British Mathematical Olympiad 1996-97 — Round 1
family: bmo
year: '1997'
level: BMO Round 1
country: Regno Unito
modalita: individuale
quesito: '2'
summary: 'Sequence with a_{n+1} defined by digit sum; find a_{1997}'
qa_score: '5'
topics:
  - topic_aritmetica
methods:
  - method_ricorsione
  - method_induzione
skills:
  - skill_riconoscimento_pattern
  - skill_manipolazione_algebrica
tags:
  - kg/quesito
  - paese/regno-unito
  - comp/bmo
  - anno/1997
  - livello/BMO-Round-1
  - topic/aritmetica
  - gara/individuale
---

*Sequence with a_{n+1} defined by digit sum; find a_{1997}*

> For positive integers $n \geq 1$, define the sequence $a_1, a_2, a_3, \ldots$ by
> $$a_1 = 1, \quad a_{n+1} = a_n + \frac{n+1}{\text{largest prime factor of }(n+1)}, \quad n \geq 1.$$
> Determine the value of $a_{1997}$.

**Topic:** [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Metodo:** [[method_ricorsione|Ricorsione]], [[method_induzione|Induzione]]
**Abilita:** [[skill_riconoscimento_pattern|Riconoscimento di pattern]], [[skill_manipolazione_algebrica|Manipolazione algebrica]]
**Area:** [[Aritmetica e Teoria dei Numeri]]
**Fonte:** apri PDF

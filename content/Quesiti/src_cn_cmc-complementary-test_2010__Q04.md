---
title: China Mathematical Competition (Complementary Test) 2010 — Quesito 4
tipo: quesito
quesito_id: quesito_src_cn_cmc-complementary-test_2010__Q04
parent: src_cn_cmc-complementary-test_2010
competition: China Mathematical Competition (Complementary Test) 2010
family: cmc-complementary-test
year: '2010'
level: China Mathematical Competition (Complementary Test)
country: Cina
modalita: individuale
quesito: '4'
summary: Count valid code-sets for cipher lock on regular n-gon
qa_score: '3'
topics:
  - topic_combinatoria
methods:
  - method_conteggio
  - method_casework
skills:
  - skill_conteggio_sistematico
  - skill_riconoscimento_pattern
tags:
  - kg/quesito
  - paese/cina
  - comp/cmc-complementary-test
  - anno/2010
  - livello/China-Mathematical-Competition-(Complementary-Test)
  - topic/combinatoria
  - gara/individuale
---

*Count valid code-sets for cipher lock on regular n-gon*

> (30 marks) The code setting of a cipher lock is established on an $n$-regular-polygon with vertices $A_1, A_2, \ldots, A_n$, each vertex is assigned a number ($1$ or $0$) and a color (red or blue), such that either the numbers or the colors on each pair of adjacent vertices are the same. We ask, How many code-sets can be realized for this lock?

**Topic:** [[topic_combinatoria|Combinatoria]]
**Metodo:** [[method_conteggio|Conteggio]], [[method_casework|Casework]]
**Abilita:** [[skill_conteggio_sistematico|Conteggio sistematico]], [[skill_riconoscimento_pattern|Riconoscimento di pattern]]
**Area:** [[Combinatoria, Logica e Probabilita]]
**Fonte:** [apri libro PDF](https://drive.google.com/file/d/1cjmD7lTT4vb3nB3ylcVCHeHucUE2rNI0/view)

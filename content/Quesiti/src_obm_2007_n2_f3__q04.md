...
fase: '3'
quesito: '4'
summary: 'Primes p,q with (p²+q²)/(p+q) integer implies that value is prime'
qa_score: '5'
topics:
  - topic_aritmetica
methods:
...
> Show that if $p, q$ are positive prime integers such that $r = \dfrac{p^2 + q^2}{p + q}$ is an integer, then $r$ is prime.

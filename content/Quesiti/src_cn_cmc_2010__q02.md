---
title: China Mathematical Competition 2010 — Quesito 2
tipo: quesito
quesito_id: quesito_src_cn_cmc_2010__Q02
parent: src_cn_cmc_2010
competition: China Mathematical Competition 2010
family: cmc
year: '2010'
level: China Mathematical Competition
country: Cina
modalita: individuale
quesito: '2'
summary: Range of parameter a so that a trigonometric expression has minimum -3
qa_score: '2'
answer: '$-\frac{3}{2} \le a \le 12$'
topics:
  - topic_trigonometria
  - topic_algebra
methods:
  - method_casework
skills:
  - skill_manipolazione_algebrica
  - skill_casework_accurato
tags:
  - kg/quesito
  - paese/cina
  - comp/cmc
  - anno/2010
  - livello/China-Mathematical-Competition
  - topic/trigonometria
  - topic/algebra
  - gara/individuale
  - qa/da-verificare
---

*Range of parameter a so that a trigonometric expression has minimum -3*

> The minimum of $y = (a\cos^2 x - 3)\sin x$ is $-3$. Then the range of real number $a$ is ________.

**Topic:** [[topic_trigonometria|Trigonometria]], [[topic_algebra|Algebra]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_manipolazione_algebrica|Manipolazione algebrica]], [[skill_casework_accurato|Casework accurato]]
**Area:** [[Geometria]], [[Algebra e Analisi]]
**Risposta:** $-\frac{3}{2} \le a \le 12$
**Fonte:** [apri libro PDF](https://drive.google.com/file/d/1cjmD7lTT4vb3nB3ylcVCHeHucUE2rNI0/view)

> [!warning] Estratto da verificare con la fonte.

---
title: Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1 — Quesito 10
tipo: quesito
quesito_id: quesito_src_obm_2004_n1_f1__Q10
parent: src_obm_2004_n1_f1
competition: Olimpíada Brasileira de Matemática 2004 — Nível 1 — Fase 1
family: obm
year: '2004'
level: OBM Nível 1
country: Brasile
modalita: individuale
nivel: '1'
fase: '1'
quesito: '10'
summary: Which garden fence project costs the most to build?
qa_score: '3'
answer: E
topics:
  - topic_geometria_piana
methods:
  - method_casework
skills:
  - skill_ragionamento_geometrico
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2004
  - livello/OBM-Nível-1
  - nivel/1
  - fase/1
  - topic/geometria_piana
  - gara/individuale
---

*Which garden fence project costs the most to build?*

> An architect presents to his client five different floor plans for an irregular terrain to be used as a garden, where the dark lines represent the fence that must be built to protect the flowers. The light regions in all cases are rectangular and the type of fence is the same in all cases. Which project will have the highest construction cost for the fence?
> 
> (A) Project A \quad (B) Project B \quad (C) Project C \quad (D) Project D \quad (E) Project E

![[src_obm_2004_n1_f1__Q10.png]]

**Topic:** [[topic_geometria_piana|Geometria piana]]
**Metodo:** [[method_casework|Casework]]
**Abilita:** [[skill_ragionamento_geometrico|Ragionamento geometrico]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Geometria]]
**Risposta:** E
**Fonte:** [apri PDF](https://drive.google.com/file/d/1-5klwGzekjemeBkehc3fYHvu2LhwtAV5/view)

---
tipo: quesito
quesito_id: quesito_src_obm_2007_n3_f1__Q21
parent: src_obm_2007_n3_f1
competition: Olimpíada Brasileira de Matemática 2007 — Nível 3 — Fase 1
family: obm
year: '2007'
level: OBM Nível 3
country: Brasile
modalita: individuale
nivel: '3'
fase: '1'
quesito: '21'
summary: At which listed time is the angle between clock hands smallest
qa_score: '5'
answer: E
topics:
  - topic_geometria_piana
  - topic_aritmetica
skills:
  - skill_stima
  - skill_lettura_attenta
tags:
  - kg/quesito
  - paese/brasile
  - comp/obm
  - anno/2007
  - livello/OBM-Nível-3
  - nivel/3
  - fase/1
  - topic/geometria_piana
  - topic/aritmetica
  - gara/individuale
---

# Olimpíada Brasileira de Matemática 2007 — Nível 3 — Fase 1 — Quesito 21

*At which listed time is the angle between clock hands smallest*

> Determine at which of the times below the angle determined by the hands of a clock is smallest.
> 
> (A) $02\mathrm{h}30$ \quad (B) $06\mathrm{h}20$ \quad (C) $05\mathrm{h}40$ \quad (D) $08\mathrm{h}50$ \quad (E) $09\mathrm{h}55$

**Topic:** [[topic_geometria_piana|Geometria piana]], [[topic_aritmetica|Aritmetica / Teoria dei Numeri]]
**Abilita:** [[skill_stima|Stima]], [[skill_lettura_attenta|Lettura attenta]]
**Area:** [[Geometria]], [[Aritmetica e Teoria dei Numeri]]
**Risposta:** E
**Fonte:** apri PDF
